...
export class DriveLeftMenu {
  ...
  readonly leftMenu = (): Locator => this.page.locator('.antiPanel-wrap__content.clubNavPanel-container')
  ...
  readonly buttonDriveInTree = (drive: Drive): Locator =>
    this.treeDrives().locator('.clubNavGroup-container').getByRole('button', { name: drive.name })

  readonly driveContainer = (drive: Drive): Locator =>
    this.treeDrives()
      .locator('.clubNavGroup-container')
      .filter({
