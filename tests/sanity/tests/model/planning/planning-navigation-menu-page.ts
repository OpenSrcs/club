...
export class PlanningNavigationMenuPage {
  ...
  readonly buttonToDoAll = (): Locator =>
    this.page.locator('button[class*="clubNavItem-container"] span[class*="clubNavItem-label"]', {
      hasText: 'All'
    })
  ...
  readonly buttonToDoPlanned = (): Locator =>
    this.page.locator('button[class*="clubNavItem-container"] span[class*="clubNavItem-label"]:text-is("Planned")')

  readonly accordionContainerToDoUnplanned = (): Locator =>
    this.page.locator('div.toDos-container div.clubAccordionItem-container', { hasText: 'Unplanned' })
  ...
  async compareCountersUnplannedToDos (): Promise<void> {
    const navCount = parseInt(
      await this.buttonToDoUnplanned().locator('xpath=..').locator('span.clubNavItem-count').innerText(),
      10
    )
    const accCount = await this.accordionContainerToDoUnplanned().locator('button.clubToDoLine-container').count()
