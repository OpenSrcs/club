...
export class SidebarPage extends CommonPage {
  ...
  contentHeaderByTitle = (title: string): Locator =>
    this.content().locator(`.clubHeader-titleGroup:has-text("${title}")`)

  contentCloseButton = (): Locator => this.content().locator('.clubHeader-container button.iconOnly').last()
  ...
  plannerSidebarNextDayButton = (): Locator =>
    this.sidebar().locator('.clubHeader-buttonsGroup').getByRole('button').last()
