...
export class SettingsPage extends CommonPage {
  ...
  spaceTypesHeader = (): Locator => this.page.locator('button.clubNavGroup-header', { hasText: 'Space types' })
  ...
  spaceTypeButton = (name: string, category?: string): Locator =>
    this.page.locator('div#navGroup-spaceTypes button.clubTaskNavLink-container', {
      hasText: category !== undefined ? `${name} ${category}` : name
    })

  breadcrumbButton = (hasText: string): Locator => this.page.locator('button.clubBreadcrumb-container', { hasText })
  stateButton = (name: string): Locator =>
    this.page
      .locator('div.clubTableAttr-header', { hasText: 'Process states' })
      .locator('xpath=..')
      .locator('button.clubTableAttr-content__row', { hasText: name })

  statusNameInput = (): Locator =>
    this.page.locator('div.clubModal-container.type-aside', { hasText: 'Edit state' }).getByPlaceholder('Status name')

  selectColorButton = (name: string): Locator =>
    this.page.locator('div.clubTableAttr-container', { hasText: 'Color' }).locator(`div.color[data-id="${name}"]`)
  ...
  taskTypeRow = (value: string): Locator =>
    this.page
      .locator('div.clubTableAttr-header', { hasText: 'Task types' })
      .locator('xpath=..')
      .locator('div.clubTableAttr-content button.clubTableAttr-content__row', { hasText: value })

  addTaskTypeButton = (): Locator =>
    this.page.locator('div.clubTableAttr-header', { hasText: 'Task types' }).locator('button[data-id="btnAdd"]')

  taskNameInput = (): Locator => this.page.getByPlaceholder('Task name *')
  taskTypeButton = (): Locator =>
    this.page.locator('div.clubModal-content__settingsSet-line', { hasText: 'Task type' }).locator('button')

  asideFooterButton = (hasText: string): Locator =>
    this.page.locator('div.clubModal-container.type-aside div.clubModal-footer button', { hasText })

  buttonRoleInComponent = (hasText: string): Locator =>
    this.page.locator('div.clubComponent-content > div.flex-row-center', { hasText }).locator('button')
