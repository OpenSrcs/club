...
export class DocumentsPage extends CommonPage {
  ...
  readonly buttonDocumentWrapper = (name: string): Locator =>
    this.page.locator(`button.clubNavItem-container:has-text("${name}")`)
  ...
  readonly breadcrumbsByDocumentParent = (parentDocumentTitle: string): Locator =>
    this.page.locator(`.clubHeader-titleGroup:has-text("${parentDocumentTitle}")`)
  ...
  readonly divTeamspacesParent = (): Locator =>
    this.page.locator('button.clubNavGroup-header', { hasText: 'Teamspaces' })

  readonly buttonTeamspaces = (): Locator =>
    this.page.locator('button.clubNavItem-container', { hasText: 'Teamspaces' })

  readonly rowTeamspace = (hasText: string): Locator =>
    this.page.locator('div.clubComponent td ', { hasText }).locator('xpath=..')

  readonly buttonJoinTeamspace = (hasText: string): Locator =>
    this.page.locator('div.clubComponent td ', { hasText }).locator('xpath=..').locator('button[type="submit"]')
  ...
  async openTeamspace (name: string): Promise<void> {
    const classes = await this.page
      .locator('button.clubNavGroup-header span[class*="label"]', { hasText: name })
      .locator('xpath=../..')
      .getAttribute('class')
  ...
  async checkTeamspaceExist (name: string): Promise<void> {
    await expect(
      this.page.locator('div[class*="clubNavGroup-content"] span[class*="label"]', { hasText: name })
    ).toHaveCount(1)
  }

  async checkTeamspaceNotExist (name: string): Promise<void> {
    await expect(
      this.page.locator('button[class*="clubNavGroup-header"] span[class*="label"]', { hasText: name })
    ).toHaveCount(0)
  }

  async moreActionTeamspace (name: string, action: string): Promise<void> {
    await this.page.locator('button.clubNavGroup-header span[class*="label"]', { hasText: name }).hover()
    await this.page
      .locator(`xpath=//span[text()="${name}"]/../../div[@class="clubNavGroup-header__tools"]/button[last()]`)
      .click()
  ...
  async openDocumentForTeamspace (spaceName: string, documentName: string): Promise<void> {
    await this.page
      .locator('button.clubNavGroup-header span[class*="label"]', { hasText: spaceName })
      .locator('xpath=../../following-sibling::div[1]')
      .locator('button.clubNavItem-container span[class*="label"]', { hasText: documentName })
      .click()
  }
  ...
  async moreActionsOnDocument (documentName: string, action: string): Promise<void> {
    await this.page.locator('button.clubNavItem-container span[class*="label"]', { hasText: documentName }).hover()
    await this.page
      .locator('button.clubNavItem-container > span[class*="label"]', { hasText: documentName })
      .locator('xpath=..')
      .locator('div.clubNavItem-actions > button:last-child')
      .click()
    await this.selectFromDropdown(this.page, action)
  }
