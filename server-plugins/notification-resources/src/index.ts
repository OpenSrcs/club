...
import activity, { ActivityMessage, DocUpdateMessage } from '@opensrcs/activity'
import { Analytics } from '@opensrcs/analytics'
import chunter, { ChatMessage } from '@opensrcs/chunter'
import contact, { Employee, type Person } from '@opensrcs/contact'
import core, {
  AccountUuid,
  AnyAttribute,
  ArrOf,
...
import { getResource, translate } from '@opensrcs/club'
import { getAccountBySocialId, getEmployeesBySocialIds } from '@opensrcs/server-contact'
import { type TriggerControl } from '@opensrcs/server-core'
import { NOTIFICATION_BODY_SIZE, ReceiverInfo, SenderInfo } from '@opensrcs/server-notification'
import { markupToText, stripTags } from '@opensrcs/text-core'
...
export async function getContentByTemplate (
  ...
  const app = control.branding?.title ?? 'club'
