...
find . -name "package.json" -not -path "*/node_modules/*" -type f | while read -r file; do
    if grep -q '"name": "@opensrcs/' "$file"; then
        echo "$file"
    fi
...
# Create filtered external package.json excluding @opensrcs packages
jq '{"name": "external-dependencies", "dependencies": (.dependencies | with_entries(select(.key | startswith("@opensrcs/") | not)))}' "$COMBINED_PACKAGE" > "$EXTERNAL_PACKAGE"
