...
/**
 * This script validates that all @opensrcs packages use the same versions
 ...
 * and only validates @opensrcs packages.
 * 
...
/**
 * Parse pnpm-lock.yaml to extract resolved @opensrcs package versions
...
function parseLockfile() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Parse @opensrcs package entries like '  @opensrcs/club@0.7.3:' or '@opensrcs/analytics@0.7.4:'
    ...
    // Parse dependency lines like "      '@opensrcs/club': 0.7.3"
    // This tells us that currentPackageName@currentPackageVersion depends on @opensrcs/club@0.7.3
    if (inDependenciesSection && currentPackageName && currentPackageVersion) {
      const depMatch = line.match(/^\s{6}'?(@opensrcs\/[^']+)'?:\s+([0-9.]+)/)
      if (depMatch) {
...
/**
 * Extract @opensrcs dependencies from package.json
...
function buildDependencyMap(projects) {
  console.log('🔍 Scanning all packages for @opensrcs dependencies...\n')
  ...
  console.log(`✅ Found ${totalDependencies} @opensrcs dependencies\n`)
...
function displayLockfileMismatches(mismatches) {
  ...
  console.log('━'.repeat(80))
  console.log()
  console.log('The following @opensrcs packages have multiple resolved versions')
 ...
 */
function main() {
  console.log('🚀 Checking @opensrcs dependency versions...\n')
  ...
  console.log(`✅ Found ${Object.keys(lockfileVersions).length} unique @opensrcs packages in lockfile`)
  ...
  if (!hasErrors) {
    console.log('━'.repeat(80))
    console.log('✅ SUCCESS - All @opensrcs dependencies use consistent versions!')
    console.log('━'.repeat(80))
    console.log()
