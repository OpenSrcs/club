...
The OpenSrcs Club is a robust framework designed to accelerate the development of business applications, such as CRM systems.
This repository includes several applications, such as Chat, Project Management, CRM, HRM, and ATS.
Various teams are building products on top of the Platform, including [club](https://club.opensrcs.org)

## Self-Hosting

If you're primarily interested in self-hosting club without the intention to modify or contribute to its development, please use [club-selfhost](https://github.com/opensrcs/club-selfhost).
This project offers a convenient method to host club using `docker`, designed for ease of use and quick setup. Explore this option to effortlessly enjoy club on your own server.

## API Client

If you want to interact with club programmatically, check out our [API Client](foundations/core/packages/api-client) documentation. The API client provides a typed interface for all club operations and can be used to build integrations and custom applications.
...
> **Note:** For resource-constrained machines, you can use the minified variants `rush docker:min` and `rush docker:up:min` to build and run only the required services (excludes clubpulse, redis, process, backup, rating, preview, link-preview, elastic, fulltext, payment, stats, print, sign, clubgun, clubkvs).
...
```plain
127.0.0.1 club.local
::1 club.local
```

Accessing the URL <http://club.local:8087> will lead you to the app in development mode.
