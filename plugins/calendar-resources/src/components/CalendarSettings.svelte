<script lang="ts">
  import core, { getCurrentAccount, groupByArray, Ref } from '@opensrcs/core'
  import { createQuery, getClient } from '@opensrcs/presentation'
  import { Breadcrumb, Grid, Header, Label, Scroller, Toggle } from '@opensrcs/ui'
  import calendar from '../plugin'
  import setting from '@opensrcs/setting'
  import { Calendar, ExternalCalendar, getPrimaryCalendar, PrimaryCalendar, Visibility } from '@opensrcs/calendar'
  ...
  $: categories = groupByArray(calendars, (c) => {
    return (c as ExternalCalendar).externalUser ?? 'CLUB'
  })
