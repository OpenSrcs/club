<script lang="ts">
  import { Integration } from '@opensrcs/account-client'
  import presentation, {
    copyTextToClipboard,
    getCurrentWorkspaceUrl,
    getCurrentWorkspaceUuid
  } from '@opensrcs/presentation'
  import { Label, Modal, CheckBox, Spinner, Button, IconCopy, EditBox } from '@opensrcs/ui'
  import calendar, { caldavIntegrationKind } from '@opensrcs/calendar'
  import { createEventDispatcher, onMount } from 'svelte'
  import { slide } from 'svelte/transition'
  import { quintOut } from 'svelte/easing'
  import { getMetadata } from '@opensrcs/club'
  import { getCurrentAccount, pickPrimarySocialId, SocialId, SocialIdType } from '@opensrcs/core'
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (socialId.type !== SocialIdType.CLUB) {
...
>
  <div class="flex-col-stretch" style="gap: 1.5rem">
    ...
    <div class="flex-col-stretch flex-gap-1-5" class:accessDisabled={!accessEnabled}>
      ...
      <div class="flex-row-center flex-gap-1">
        <EditBox bind:value={clubSocialId} kind="ghost" fullSize focusable disabled={true} />
        <Button
          icon={IconCopy}
          size="small"
          kind="ghost"
