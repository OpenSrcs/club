...
-->
<script lang="ts">
  import { LoginInfo, WorkspaceLoginInfo } from '@opensrcs/login'
  import { getAccount, getAccountDisplayName, getWorkspaces, navigateToWorkspace } from '@opensrcs/login-resources'
  ...
  const action = {
    i18n: onboard.string.StartUsingClub,
    func: async () => {
      if (account != null && isWorkspaceLoginInfo(account)) {
        Analytics.handleEvent(OnboardEvents.StartClub)
