...
-->
<script lang="ts">
  import activity, { ActivityMessage, ActivityMessageViewType, DisplayActivityMessage } from '@opensrcs/activity'
  import {
    ActivityDocLink,
    ActivityMessageTemplate,
    MessageInlineAction,
    editingMessageStore
  } from '@opensrcs/activity-resources'
  import { Attachment } from '@opensrcs/attachment'
  import { AttachmentDocList, AttachmentImageSize } from '@opensrcs/attachment-resources'
  import chunter, { ChatMessage, ChatMessageViewlet } from '@opensrcs/chunter'
  import contact, { getCurrentEmployee, Person, SocialIdentity } from '@opensrcs/contact'
  import { getPersonByPersonIdCb, getSocialIdByPersonIdCb } from '@opensrcs/contact-resources'
  import { Class, Doc, Markup, Ref, Space, WithLookup } from '@opensrcs/core'
  import { getClient, MessageViewer, pendingCreatedDocs } from '@opensrcs/presentation'
  import { EmptyMarkup } from '@opensrcs/text'
  import { Action, Button, IconEdit, ShowMore } from '@opensrcs/ui'
  import view from '@opensrcs/view'
  import { getDocLinkTitle } from '@opensrcs/view-resources'
...
{:else if value && !inline}
  <ActivityMessageTemplate
    ...
    {parentMessage}
    {person}
    socialId={socialId?.type !== 'club' ? socialId : undefined}
    {showNotify}
    {isHighlighted}
    {isSelected}
    {shouldScroll}
