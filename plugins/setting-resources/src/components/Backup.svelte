...
<script lang="ts">
  ...
  import presentation, { copyTextToClipboard, getFileUrl } from '@opensrcs/presentation'
  import { Breadcrumb, Button, Expandable, Header, Label, Loading, Scroller } from '@opensrcs/ui'
  ...
  function backupArchiveName (): string {
    ...
    return `club-backup-${workspaceId}-${date}.zip`
  }
  ...
  async function downloadFullBackup (): Promise<void> {
    ...
    try {
      ...
      // into any other club setup directly.
  ...
  function copyBackupScript (): void {
    ...
    const script = generateBackupScript({
      ...
      files,
      outDir: `club-backup-${workspaceId}`,
...
<div class="clubComponent flex-no-shrink">
