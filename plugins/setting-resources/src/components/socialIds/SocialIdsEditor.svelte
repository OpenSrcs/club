...
<script lang="ts">
  ...
  import { getCurrentAccount, loginSocialTypes, notEmpty, SocialIdType } from '@opensrcs/core'
  import { getClient, hasResource } from '@opensrcs/presentation'
  import { Action, Button, createFocusManager, FocusHandler, Label, Menu, Scroller, showPopup } from '@opensrcs/ui'
  ...
  $: onlyClub = socialIds.filter((it) => it.type === SocialIdType.CLUB).length === 1
  ...
  <div class="items">
    <Scroller>
      {#each socialIds as socialId}
        ...
        {@const canRelease =
          socialId.type === SocialIdType.CLUB
            ? !onlyClub
            : loginSocialTypes.includes(socialId.type)
              ? !onlyLogin
              : true}
        {#if socialIdProvider != null && socialId.type !== SocialIdType.CLUB}
