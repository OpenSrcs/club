...
-->
<script lang="ts">
  import { AttributeEditor, MessageBox, createQuery, getClient } from '@opensrcs/presentation'
  import core, { Permission, Ref, Role, SpaceType, SpaceTypeDescriptor } from '@opensrcs/core'
  import {
    ButtonIcon,
    Icon,
    IconDelete,
  ...
  import { ObjectBoxPopup } from '@opensrcs/view-resources'
  import { deleteSpaceTypeRole } from '@opensrcs/setting'
...
{#if role !== undefined}
  <div class="clubComponent-content__container columns">
    <div class="clubComponent-content__column content">
      <Scroller align={'center'} padding={'var(--spacing-3)'} bottomPadding={'var(--spacing-3)'}>
        <div class="clubComponent-content gap">
          <div class="clubComponent-content__column-group mt-4">
            <div class="clubComponent-content__header mb-6 gap-2">
              <ButtonIcon
                icon={IconDelete}
                size="large"
                kind="secondary"
              ...
              {#if permissions.length > 0}
                <div class="clubTableAttr-content task">
                  {#each permissions as permission}
                    <div class="clubTableAttr-content__row">
                      {#if permission.icon !== undefined}
                        <div class="clubTableAttr-content__row-icon-wrapper">
                          <Icon icon={permission.icon} size="small" />
                        </div>
                      {/if}

                      <div class="clubTableAttr-content__row-label font-medium-14">
                        <Label label={permission.label} />
                      </div>

                      {#if permission.description !== undefined}
                        <div class="clubTableAttr-content__row-label grow dark font-regular-14">
