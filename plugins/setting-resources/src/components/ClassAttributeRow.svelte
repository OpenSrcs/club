...
-->
<script lang="ts">
  import core, { AnyAttribute, ArrOf, Doc, EnumOf, RefTo, Type } from '@opensrcs/core'
  import { IntlString } from '@opensrcs/club'
  import { getClient } from '@opensrcs/presentation'
  import { AnySvelteComponent, Icon, IconMoreV2, IconOpenedArrow, Label, tooltip } from '@opensrcs/ui'
...
<button class="clubTableAttr-content__row w-full" class:hovered class:selected on:contextmenu on:click>
  <button class="clubTableAttr-content__row-dragMenu">
    <IconMoreV2 size={'small'} />
  </button>
  {#if attribute.automationOnly === true}
    <div class="clubTableAttr-content__row-icon" use:tooltip={{ label: view.string.AutomationOnly }}>
  ...
  {/if}
  <div class="clubTableAttr-content__row-label font-regular-14 grow" class:accent={!attribute.hidden}>
  ...
  {/if}
  <div class="clubTableAttr-content__row-type font-medium-12">
...
<style lang="scss">
  .clubTableAttr-content__row-label {
    display: flex;
    align-items: center;
    min-width: 0;
    gap: var(--spacing-0_5);
