...
import { type Asset, type IntlString, type Resource, getResource } from '@opensrcs/club'
import {
  type ContextStore,
  MessageBox,
  contextStore,
...
import { markupToJSON } from '@opensrcs/text'
...
import workbenchPlugin from '@opensrcs/workbench'
...
export async function copyMarkdown (markdown: string, metadata?: Record<string, any>): Promise<void> {
  ...
  if (metadata !== undefined) {
    try {
      const metadataComment = `<!-- club-table-metadata:${JSON.stringify(metadata)} -->`
  ...
  try {
    if (typeof ClipboardItem !== 'undefined') {
      ...
      if (metadata !== undefined) {
        try {
          clipboardData['application/x-club-table-metadata'] = Promise.resolve(JSON.stringify(metadata))
        } catch (e) {
