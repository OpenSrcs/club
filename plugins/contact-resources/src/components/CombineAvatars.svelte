...
-->
<script lang="ts">
  import { Person as Contact } from '@opensrcs/contact'
...
{#if items !== undefined}
  <div class="clubCombineAvatars-container">
    {#if includeEmpty}
      <div class="clubCombineAvatar {size}" data-over={getDataOver(persons.length === 0, items)}>
        <EmptyAvatar {size} />
      </div>
    {/if}
    {#each persons as person, i}
      <div class="clubCombineAvatar {size}" data-over={getDataOver(persons.length === i + 1, items)}>
