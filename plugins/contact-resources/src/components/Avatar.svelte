...
<script lang="ts">
  ...
  import { getBlobURL, getClient, reduceCalls, sizeToWidth } from '@opensrcs/presentation'
  import {
    AnySvelteComponent,
    ColorDefinition,
  ...
  import { AccountUuid, type Data, PersonUuid, Ref, type WithLookup } from '@opensrcs/core'
...
<div class="flex-presenter" class:no-pointer={!clickable} use:tooltip={getPreviewPopup(person, showPreview)} on:click>
  {#if showStatus && person}
    <div class="relative">
      ...
      />
      <div
        class="clubAvatar-statusMarker {statusSize ?? size} {style}"
