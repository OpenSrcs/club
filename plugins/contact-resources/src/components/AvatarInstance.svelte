...
<script lang="ts">
  ...
  import { AnySvelteComponent, ColorDefinition, Icon, IconSize, resizeObserver } from '@opensrcs/ui'
...
{#if (size === 'full' || adaptiveName) && !url && displayName && displayName !== ''}
  ...
  <div
    bind:this={element}
    class="clubAvatar-container clubAvatarSize-{size} {variant} {style}"
...
{:else}
  ...
  >
    {#if url && !imgError}
      <img
        class="clubAvatarSize-{size} ava-image {disabled ? 'disabled' : ''} {style}"
        src={url}
        {srcset}
        alt={''}
