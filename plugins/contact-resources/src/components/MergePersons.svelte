...
<script lang="ts">
  ...
  import { Channel, Person, SocialIdentity, getName } from '@opensrcs/contact'
  import core, {
    ArrOf,
    Doc,
    DocumentUpdate,
  ...
  import { Card, createQuery, getClient, updateAttribute } from '@opensrcs/presentation'
  import { Label, Spinner, Toggle } from '@opensrcs/ui'
  import { isCollectionAttr } from '@opensrcs/view-resources'
  ...
  $: visibleResultSocialIdentities = resultSocialIdentities.filter(
    (it) => it.type !== SocialIdType.CLUB && it.type !== SocialIdType.CLUB_ASSISTANT
  )
