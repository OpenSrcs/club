...
-->
<script lang="ts">
  import contact, { Contact, Person } from '@opensrcs/contact'
  import { AssigneeBox } from '@opensrcs/contact-resources'
  import { Ref } from '@opensrcs/core'
  import { getClient } from '@opensrcs/presentation'
  import { ActionIcon, EditBox, Icon, IconDelete, resizeObserver } from '@opensrcs/ui'
  import { Room, RoomAccess, RoomType, isOffice } from '@opensrcs/love'
...
<div
  ...
  class:placed
  style={dragStyle}
  style:--club-floor-roomWidth={room.width}
  style:--club-floor-roomHeight={room.height}
  style:--club-floor-roomShadow={`var(--theme-popup-shadow), ${$shadow.x}px ${$shadow.y}px ${$shadow.r}px ${$shadow.s}px rgba(${$shadowColor.r}, ${$shadowColor.g}, ${$shadowColor.b}, ${$shadowColor.a})`}
