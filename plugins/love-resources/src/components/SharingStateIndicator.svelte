...
-->
<script lang="ts">
  import { tooltip, eventToHTMLElement, showPopup } from '@opensrcs/ui'
...
{#if $lkSessionConnected}
  {#if $screenSharingState === ScreenSharingState.Local}
    <button
      class="clubStatusBarButton mini positive positiveContent"
      class:pressed
  ...
  {:else}
    <button
      class="clubStatusBarButton mini disabled"
      class:pressed
