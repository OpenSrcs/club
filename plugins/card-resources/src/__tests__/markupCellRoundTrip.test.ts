...
jest.mock('@opensrcs/club', () => {
  const actual = jest.requireActual('@opensrcs/club')
  return {
    ...actual,
...
jest.mock('@opensrcs/presentation', () => ({
  getClient: jest.fn()
}))
...
jest.mock('@opensrcs/converter-resources', () => ({
...
// @opensrcs/converter-resources so we don't pull svelte into Jest.
...
describe('markup-in-cell round-trip', () => {
  ...
  it('reparses the exact failing user table (custom markup column with nested table) without violating the schema', () => {
                      ...
                      content: [
                        {
                          ...
                          marks: [
                            {
                              type: 'link',
                              attrs: { href: 'http://club.local:8080/workbench/t4/card/6a05579937207bd342d60ff8' }
                            }
                          ]
                        }
                      ]
    ...
    expect(cellValue).toContain('http://club.local:8080/workbench/t4/card/6a05579937207bd342d60ff8')

    const escaped = escapeMarkdownTableCellContent(cellValue)
    const outerTable =
      '| test | Type | Tags | TM 2 |\n' +
      '| --- | --- | --- | --- |\n' +
      `| [new](http://club.local:8087/workbench/t4/card/6a05606432d68883381bbeb6) | test |  | ${escaped} |\n`
