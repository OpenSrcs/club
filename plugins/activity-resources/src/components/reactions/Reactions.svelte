...
<script lang="ts">
  ...
  import { Reaction } from '@opensrcs/activity'
  import { Doc, getCurrentAccount, PersonId, Ref, Blob } from '@opensrcs/core'
  import { IconAdd, showPopup, tooltip } from '@opensrcs/ui'
  import { includesAny } from '@opensrcs/contact'
  import emojiPlugin from '@opensrcs/emoji'

  import ReactionsTooltip from './ReactionsTooltip.svelte'
  import { updateDocReactions } from '../../utils'
  import { getBlobRef } from '@opensrcs/presentation'
...
<div class="clubReactions-container">
  {#each [...reactionsPersons] as [emoji, emojiInfo]}
    <!-- svelte-ignore a11y-no-static-element-interactions -->
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <div
      class="clubReactions-button"
  ...
  {#if object && reactionsPersons.size > 0 && !readonly}
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <!-- svelte-ignore a11y-no-static-element-interactions -->
    <div class="clubReactions-button withoutBackground" class:opened on:click={openEmojiPalette}>
      <IconAdd size="small" />
    </div>
  {/if}
</div>
