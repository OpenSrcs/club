...
-->
<script lang="ts">
  import { SortingOrder, WithLookup } from '@opensrcs/core'
  import { createQuery, getClient } from '@opensrcs/presentation'
  import tags from '@opensrcs/tags'
  import { Component, IconMoreV2, Spinner, showPanel, Icon } from '@opensrcs/ui'
  import { showMenu } from '@opensrcs/view-resources'
  import time, { ToDo, ToDoPriority, WorkSlot } from '@opensrcs/time'
...
>
  <div class="flex-row-center flex-grow flex-gap-2">
    <div class="clubToDoLine-statusPriority" class:isNew>
      {#if updating !== undefined}
        <Spinner size={'small'} />
      {:else}
        <button class="clubToDoLine-dragbox" on:contextmenu={onMenuClick}>
    ...
    <div class="clubToDoLine-title text-left font-regular-14 line-height-auto overflow-label">
      {todo.title}
    </div>
  </div>
  ...
  <div class="clubToDoLine-checkbox" class:updating>
