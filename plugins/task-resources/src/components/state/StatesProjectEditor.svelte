...
-->
<script lang="ts">
  import core, { IdMap, Ref, Status, StatusCategory, toIdMap } from '@opensrcs/core'
  import { Asset } from '@opensrcs/club'
  import { createQuery } from '@opensrcs/presentation'
  import { settingsStore } from '@opensrcs/setting-resources'
  import { ProjectStatus, ProjectType, TaskType } from '@opensrcs/task'
  import { IconMoreV2, IconOpenedArrow, Label } from '@opensrcs/ui'
  import { ObjectPresenter, statusStore } from '@opensrcs/view-resources'
...
{#each categories as cat (cat._id)}
  ...
  <div class="clubTableAttr-content class withTitle">
    <div class="clubTableAttr-content__title">
      <Label label={cat.label} />
    </div>
    <div class="clubTableAttr-content__wrapper">
      {#if states.length === 0}
        <!-- Placeholder для пустой категории -->
        <button
          class="clubTableAttr-content__row disableMouseOver"
      ...
      {#each states as state, i (state._id)}
        ...
        <button
          ...
          class="clubTableAttr-content__row"
        ...
        >
          <button class="clubTableAttr-content__row-dragMenu" on:click|stopPropagation={() => {}}>
          ...
          />
          <div class="clubTableAttr-content__row-arrow">
