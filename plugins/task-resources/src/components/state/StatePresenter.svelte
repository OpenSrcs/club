...
-->
<script lang="ts">
  import core, { IdMap, Ref, Status, StatusCategory } from '@opensrcs/core'
  import { Asset } from '@opensrcs/club'
  import { IconWithEmoji, getClient, reduceCalls } from '@opensrcs/presentation'
  import task, { Project, ProjectType, TaskType } from '@opensrcs/task'
  import {
    ColorDefinition,
    Icon,
    IconSize,
...
{#if value}
  {#if kind === 'table-attrs'}
    <button class="clubTableAttr-content__row-icon-wrapper" on:click>
      {#if icon != null}
    ...
    <span class="clubTableAttr-content__row-label font-medium-12 uppercase grow overflow-label">
      {value.name}
    </span>
  {:else}
