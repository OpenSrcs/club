...
<script lang="ts">
  ...
  import presentation, { Card, getCurrentWorkspaceUuid } from '@opensrcs/presentation'
  import { Label, Loading } from '@opensrcs/ui'
  import { type Integration } from '@opensrcs/account-client'
  import { isWorkspaceIntegration } from '@opensrcs/integration-client'
  ...
  import { buildSocialIdString, getCurrentAccount, SocialIdType, type PersonId } from '@opensrcs/core'
  import ClubAssistant from './icons/ClubAssistant.svelte'
  ...
  onMount(async () => {
    try {
      ...
      isLoading = false
      console.error('Failed to find/create club assistant social id/integration:', err)
