...
<script lang="ts">
  ...
  import presentation, { Card, getClient, getCurrentWorkspaceUuid, SpaceSelector } from '@opensrcs/presentation'
  import { DropdownLabels, Icon, Label, Loading } from '@opensrcs/ui'
  import { type Integration } from '@opensrcs/account-client'
  import { isWorkspaceIntegration, getIntegrationConfig } from '@opensrcs/integration-client'
  ...
  import ClubMail from './icons/ClubMail.svelte'
...
<Card
  label={clubMail.string.Configure}
  okAction={async () => {
    await apply()
    dispatch('close')
  }}
...
>
  <svelte:fragment slot="title">
    <div class="flex-row-center gap-2">
      <ClubMail size="medium" />
      <span class="text-normal">
        <Label label={clubMail.string.Configure} />
      </span>
    </div>
  </svelte:fragment>
  <div class="flex-col min-w-100 flex-gap-4" transition:fade={{ duration: 300 }}>
    ...
    {:else if mailboxItems.length === 0}
      <div class="flex-row-center">
        <Label label={clubMail.string.MailboxesNotConfigured} />
      </div>
      <div class="flex-row-center">
        <Label label={clubMail.string.ConfigureMailBoxes} />
      </div>
    {:else}
      <div class="flex-between flex-gap-4">
        <Label label={clubMail.string.Mailbox} />
        <DropdownLabels
          items={mailboxItems}
      ...
      <div class="flex-between flex-gap-4">
        <Label label={clubMail.string.ChannelSpace} />
        <SpaceSelector
    ...
    <div class="flex-row-center max-w-80 pr-4">
      {#if personSpace === selectedSpace && mailboxItems.length > 0}
        ...
        <span class="text-sm ml-2">
          <Label label={clubMail.string.PersonSpaceInfo} />
        </span>
      {:else}
        <Icon size={'small'} icon={contact.icon.Contacts} />
        <span class="text-sm ml-2">
          <Label label={clubMail.string.SharedSpaceInfo} />
        </span>
      {/if}
    </div>
  </svelte:fragment>
