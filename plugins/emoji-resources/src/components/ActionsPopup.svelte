<script lang="ts">
  ...
  import emojiPlugin, { ExtendedEmoji, Emoji } from '@opensrcs/emoji'
  import { getEmojiByHexcode, getEmojiSkins } from '../utils'
  import EmojiButton from './EmojiButton.svelte'
  import { getSkinTone, unicodeEmojiStore } from '../store'
  import { Label, IconDelete, closeTooltip, ButtonBase } from '@opensrcs/ui'
...
<div class="clubPopup-container noPadding autoWidth">
  {#if haveSkins}
    {#if combinedEmoji && emojiParts?.length === 2}
      <div class="clubPopup-row disabled skins-row">
  ...
  {/if}
  {#if remove}
    {#if haveSkins}<div class="clubPopup-divider" />{/if}
    <div class="clubPopup-group">
      <button class="clubPopup-row" on:click={clickRemove}>
        <div class="clubPopup-row__icon red-color"><IconDelete size={'small'} /></div>
        <span class="clubPopup-row__label red-color"><Label label={emojiPlugin.string.Remove} /></span>
      </button>
    </div>
  {/if}
</div>
