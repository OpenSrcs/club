...
import { type Workspace } from '@opensrcs/account'
import { type JsonPatch, type ClublakeWorkspaceClient } from '@opensrcs/clublake-client'
...
import {
  ...
  type CardPeer,
  type Peer
} from '@opensrcs/communication-types'
import { type AccountClient } from '@opensrcs/account-client'
import chunter, {
  type Channel,
  type ChunterSpace,
  type ThreadMessage,
  type DirectMessage
} from '@opensrcs/chunter'
import cardPlugin, { type Card, type CardSpace, DOMAIN_CARD } from '@opensrcs/card'
import { makeRank } from '@opensrcs/rank'
import chat from '@opensrcs/chat'
import { DOMAIN_ACTIVITY } from '@opensrcs/model-activity'
import { DOMAIN_CONTACT } from '@opensrcs/model-contact'
import { markupToMarkdown } from '@opensrcs/text-markdown'
import { markupToJSON, markupToText } from '@opensrcs/text'
import activity, { type ActivityMessage } from '@opensrcs/activity'
import communication, { type Direct } from '@opensrcs/communication'
import { type Employee, formatName, type Person, type PersonSpace } from '@opensrcs/contact'
import { withRetry, DEFAULT_RETRY_OPTIONS } from '@opensrcs/retry'
...
export async function migrateWorkspaceChat (
  ...
  hierarchy: Hierarchy,
  clublake: ClublakeWorkspaceClient,
  accountClient: AccountClient,
  personUuidBySocialId: Map<PersonId, PersonUuid>
): Promise<void> {
  await migrateChannels(ctx, ws, db, client, hierarchy, clublake, accountClient, personUuidBySocialId)
  await migrateDirects(ctx, ws, db, client, hierarchy, clublake, accountClient, personUuidBySocialId)
}
  ...
  for (const doc of docs) {
    ...
    await limiter.add(async () => {
      try {
        ...
        await migrateChannel(ctx, ws, db, client, hierarchy, clublake, accountClient, personUuidBySocialId, doc)
      } catch (e) {
...
async function migrateChannel (
  ...
  await migrateMessages(ctx, ws, db, client, hierarchy, clublake, accountClient, personUuidBySocialId, doc, [card])
}
  ...
  for (const doc of docs) {
    ...
    await limiter.add(async () => {
      try {
        ...
        await migrateDirect(ctx, ws, db, client, hierarchy, clublake, accountClient, personUuidBySocialId, doc)
      } catch (e) {
...
async function migrateMessages (
  ...
  if (doc.__migratedUntil == null) {
    for (const card of cards) {
      await createGroupsBlob(clublake, card._id)
    }
  }
  ...
  for await (const nn of iterator) {
    ...
    while (messages.length >= MAX_MESSAGES_BATCH || next.length === 0) {
      ...
      for (const card of cards) {
        ...
        await insertMessages(ctx, db, ws.uuid, clublake, card, convertedMessages)
      }
    }
    prev = messages
  }
...
async function createThread (
  ...
  try {
    ...
    await createGroupsBlob(clublake, thread._id)
    const parent = {
      ...parentMessage,
      cardId: thread._id,
    ...
    await insertMessages(ctx, db, ws, clublake, thread, [parent, ...messages])
...
async function insertMessages (
  ...
  for (const chunk of chunks) {
    ...
    await clublake.patchJson(`${card._id}/messages/groups`, jsonPatches, undefined, DEFAULT_RETRY_OPTIONS)
    await clublake.putJson(`${card._id}/messages/${blobId}`, newMessagesDoc, undefined, DEFAULT_RETRY_OPTIONS)
  }
}
...
async function createGroupsBlob (clublake: ClublakeWorkspaceClient, cardId: CardID): Promise<void> {
  await clublake.putJson(`${cardId}/messages/groups`, {}, undefined, DEFAULT_RETRY_OPTIONS)
}
