...
import { getFirstName, getLastName } from '@opensrcs/contact'
import {
  systemAccountUuid,
  type BackupClient,
  type Client,
...
import { type DBDoc } from '@opensrcs/postgres/types/utils'
...
import { type BackupStorage, createFileBackupStorage, restore } from '@opensrcs/server-backup'
import { buildStorageFromConfig, storageConfigFromEnv } from '@opensrcs/server-storage'
...
export async function migrateMergedAccounts (
  ...
  try {
    ...
    const personsAccounts = await pgClient`
      ...
      WHERE new_social_key != 'email:club.ai.bot@club.opensrcs.org'
