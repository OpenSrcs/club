...
import accountPlugin, {
  ...
  type AccountDB,
  type Workspace
} from '@opensrcs/account'
import {
  getMongoAccountDB,
  type Account as OldAccount,
  type Workspace as OldWorkspace
} from '@opensrcs/account-service'
import { getWorkspaceClient as getClublakeClient } from '@opensrcs/clublake-client'
...
import {
  ...
  restore
} from '@opensrcs/server-backup'
import serverClientPlugin, { BlobClient, getAccountClient, getTransactorEndpoint } from '@opensrcs/server-client'
import {
...
import serverToken, { decodeToken, generateToken } from '@opensrcs/server-token'
import { createWorkspace, upgradeWorkspace } from '@opensrcs/workspace-service'

import { faker } from '@faker-js/faker'
import { getPlatformQueue } from '@opensrcs/kafka'
import { buildStorageFromConfig, createStorageFromConfig, storageConfigFromEnv } from '@opensrcs/server-storage'
import { program, type Command } from 'commander'
import { updateField } from './workspace'

import { RatingCalculator, ratingEvents, type QueueRatingMessage } from '@opensrcs/pod-rating'
...
import { consoleModelLogger, type MigrateOperation } from '@opensrcs/model'
import {
  createMongoAdapter,
...
import { getModelVersion } from '@opensrcs/model-all'
import {
  QueueTopic,
  workspaceEvents,
  type Pipeline,
...
import { createRestClient } from '@opensrcs/api-client'
import { type CardID } from '@opensrcs/communication-types'
import { connect, sendTransactorEvent } from '@opensrcs/server-tool'
...
export function devTool (
  ...
  program
    ...
    .option('-b, --branding <branding>', 'Branding to restore workspace with', 'club')
  ...
  program
    ...
    .action(async (cmd: { workspace?: WorkspaceUuid }) => {
      const { dbUrl, txes } = prepareTools()
      const clublakeUrl = process.env.CLUBLAKE_URL ?? ''

      const workspace = cmd.workspace
      console.log('Workspace', workspace)

      if (clublakeUrl === '') {
        throw new Error('CLUBLAKE_URL should be specified')
      }
      ...
      await withAccountDatabase(async (accountDb) => {
        ...
        for (const wss of workspaces) {
          try {
            ...
            if (ws == null) continue
            const clublake = getClublakeClient(clublakeUrl, ws.uuid, token)
