...
//
import { concatLink, TxOperations } from '@opensrcs/core'
import {
  ClickupImporter,
  ...
  importNotion,
  type Logger
} from '@opensrcs/importer'
import { setMetadata } from '@opensrcs/club'
import serverClientPlugin, { createClient, getAccountClient } from '@opensrcs/server-client'
...
export function importTool (): void {
  ...
  // import /home/anna/xored/club/platform/dev/import-tool/src/club/example-workspace --workspace ws1 --user user1 --password 1234
  program
    .command('import <dir>')
    .description('import issues in Unified club Format')
    .requiredOption('-u, --user <user>', 'user')
    .requiredOption('-p, --password <password>', 'password')
    .requiredOption('-w, --workspace <workspace>', 'workspace url where the documents should be imported to')
    .action(async (dir: string, cmd) => {
      const { workspace, user, password } = cmd
      await authorize(user, password, workspace, async (client, uploader) => {
        const importer = new ClubFormatImporter(client, uploader, new ConsoleLogger())
  ...
  program
    ...
    .description('convert QMS document into Unified club Format')
    .action(async (dir: string, cmd) => {
