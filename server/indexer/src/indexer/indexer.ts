...
import attachmentPlugin, { type Attachment } from '@opensrcs/attachment'
...
import drivePlugin, { type FileVersion } from '@opensrcs/drive'
import type {
  ConsumerControl,
  ContentTextAdapter,
  DbAdapter,
...
import { RateLimiter, SessionDataImpl } from '@opensrcs/server-core'
import { jsonToText, markupToJSON, markupToText } from '@opensrcs/text'
import card, { type Card } from '@opensrcs/card'
...
import {
  ...
  loadMessages,
  loadMessagesGroups
} from '@opensrcs/communication-shared'
import { markdownToMarkup } from '@opensrcs/text-markdown'
...
export class FullTextIndexPipeline implements FullTextPipeline {
  ...
  constructor (
    ...
    readonly clublake: ClublakeWorkspaceClient,
  ...
  async indexCommunication (
    ...
    let messagesGroups = []
    try {
      messagesGroups = await loadMessagesGroups(this.clublake, card._id)
    } catch (err: any) {
