...
import accountEn from '@opensrcs/account/lang/en.json'
import accountRu from '@opensrcs/account/lang/ru.json'
import { Analytics } from '@opensrcs/analytics'
import { registerProviders } from '@opensrcs/auth-providers'
import { metricsAggregate, type Branding, type BrandingMap, type MeasureContext } from '@opensrcs/core'
import platform, { Severity, Status, addStringsLoader, setMetadata, unknownStatus } from '@opensrcs/club'
import serverToken, { decodeToken, decodeTokenVerbose, generateToken } from '@opensrcs/server-token'
  ...
  if (dbUrl.startsWith('mongodb://')) {
    if (process.env.PROCEED_V7_MONGO !== 'true') {
      console.error(`
        ...
        https://github.com/opensrcs/club-selfhost
