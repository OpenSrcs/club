...
import { MeasureContext, Blob as PlatformBlob, WorkspaceIds, metricsAggregate, type Ref } from '@opensrcs/core'
import platform, { PlatformError } from '@opensrcs/club'
import { TokenError, decodeToken } from '@opensrcs/server-token'
import { StorageAdapter } from '@opensrcs/storage'
...
export function start (
  ...
  app.get('/config.json', async (req, res) => {
    const data = {
      ...
      CLUBLAKE_URL: config.clublakeUrl,
  ...
  const handleDelete = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // TODO: We need to allow delete only of user attached documents. (https://front.club.opensrcs.org/workbench/platform/tracker/TSK-1081)
