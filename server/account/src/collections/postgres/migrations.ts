...
function getV1Migration (ns: string, flavor: DBFlavor): [string, string] {
  ...
  return [
    ...
    /* ======= T Y P E S ======= */
    CREATE TYPE ${ns}.social_id_type AS ENUM ('email', 'github', 'google', 'phone', 'oidc', 'club', 'telegram');
...
function getV16Migration (ns: string, flavor: DBFlavor): [string, string] {
  ...
  const addValueSql =
    flavor === 'postgres'
      ? `
    -- Add club-assistant value to social_id_type enum (PostgreSQL)
    DO $$     BEGIN
        IF NOT EXISTS (
            SELECT 1 FROM pg_enum
            WHERE enumlabel = 'club-assistant'
            ...
            ALTER TYPE ${ns}.social_id_type ADD VALUE 'club-assistant';
        END IF;
    END $$;
    `
      : `
    -- Add club-assistant value to social_id_type enum (CockroachDB)
    ALTER TYPE ${ns}.social_id_type ADD VALUE IF NOT EXISTS 'club-assistant';
    `

  return ['account_db_v16_add_club_assistant_social_id_type', addValueSql]
}
