...
import { SocialIdType, type MeasureContext, type PersonUuid } from '@opensrcs/core'
import platform, { getMetadata } from '@opensrcs/club'
import { decodeTokenVerbose, generateToken } from '@opensrcs/server-token'
...
describe('checkHasPassword', () => {
  ...
  test('returns error for invalid/expired token', async () => {
    const { TokenError } = jest.requireMock('@opensrcs/server-token')
