...
import platform, { getMetadata, PlatformError, Severity, Status } from '@opensrcs/club'
import { decodeTokenVerbose, generateToken, TokenError } from '@opensrcs/server-token'
...
jest.mock('@opensrcs/server-token', () => ({
  TokenError: jest.requireActual('@opensrcs/server-token').TokenError,
  decodeTokenVerbose: jest.fn(),
  generateToken: jest.fn()
}))

// Mock analytics
jest.mock('@opensrcs/analytics', () => ({
  Analytics: {
    handleError: jest.fn()
  }
}))
...
describe('account utils', () => {
  ...
  describe('signUpByEmail', () => {
    ...
    test('should create CLUB social id when creating account', async () => {
      const email = 'new@example.com'
      const password = 'password123'
      const firstName = 'John'
      const lastName = 'Doe'
      ...
      // Verify CLUB social id creation
      expect(mockDb.socialId.insertOne).toHaveBeenCalledWith(
        expect.objectContaining({
