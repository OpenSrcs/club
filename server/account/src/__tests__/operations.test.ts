...
import platform, { PlatformError, Status, Severity, getMetadata } from '@opensrcs/club'
import { decodeToken, decodeTokenVerbose } from '@opensrcs/server-token'
  ...
  describe('registration operations', () => {
    ...
    describe('signUp', () => {
      test('should create account when email service not configured', async () => {
        ...
        jest.spyOn(utils, 'confirmClubIds').mockResolvedValue()
        ;(mockDb.person.findOne as jest.Mock).mockResolvedValue(mockPerson)
        ...
        expect(utils.confirmClubIds).toHaveBeenCalledWith(mockCtx, mockDb, mockAccountId)
