...
import platform, { getMetadata, PlatformError, Severity, Status, unknownError } from '@opensrcs/club'
import { decodeTokenVerbose } from '@opensrcs/server-token'
...
export async function getPersonInfo (
  ...
  const { extra } = decodeTokenVerbose(ctx, token)
  verifyAllowedServices(['workspace', 'tool', 'gmail', 'club-mail', 'export'], extra)
...
export async function addSocialIdToPerson (
  ...
  if (extra?.admin !== 'true') {
    verifyAllowedServices(
      ['github', 'telegram-bot', 'gmail', 'tool', 'workspace', 'clubgram', 'google-calendar', 'ai-assistant'],
      extra
    )
  }
...
export async function findFullSocialIds (
  ...
  const { extra } = decodeTokenVerbose(ctx, token)
  verifyAllowedServices(['gmail', 'tool', 'workspace', 'club-mail', 'rating'], extra)
