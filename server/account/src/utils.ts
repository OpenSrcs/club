...
import { getMongoClient } from '@opensrcs/mongo' // TODO: get rid of this import later
import platform, { getMetadata, PlatformError, Severity, Status, translate } from '@opensrcs/club'
import { getDBClient, setDBExtraOptions } from '@opensrcs/postgres'
...
/**
 * Creates an account and a club social id for the specified person.
 * Returns the _id of the newly created club social id.
 */
export async function createAccount (
  ...
  // Create club social id and account
  const socialId = await db.socialId.insertOne({
    type: SocialIdType.CLUB,
    value: personUuid,
    personUuid,
...
export async function signUpByGrant (
  ...
  // If there's no account there should be no club social id associated with the person if it existed
...
export async function createWorkspaceRecord (
  ...
  const brandingKey = branding?.key ?? 'club'
...
export async function confirmClubIds (ctx: MeasureContext, db: AccountDB, account: AccountUuid): Promise<void> {
  const clubSocialIds = await db.socialId.find({ personUuid: account, type: SocialIdType.CLUB, verifiedOn: null })
  for (const clubSocialId of clubSocialIds) {
    await db.socialId.update({ _id: clubSocialId._id }, { verifiedOn: Date.now() })
  }
}
...
export async function loginOrSignUpWithProvider (
  ...
  try {
    ...
    await confirmClubIds(ctx, db, personUuid as AccountUuid)
