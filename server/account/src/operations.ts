...
import platform, { getMetadata, PlatformError, Severity, Status, translate } from '@opensrcs/club'
import {
  decodeToken,
  decodeTokenVerbose,
  generateToken,
...
export async function signUp (
  ...
  if (forceConfirmation) {
    ...
    await confirmClubIds(ctx, db, account)
  }
  ...
  try {
    ...
    if (action !== 'verify') {
      ...
      if (targetAccount == null) {
        ...
        await confirmClubIds(ctx, db, targetAccount.uuid)
...
export async function generate2faSecret (
  ...
  const app = branding?.title ?? getMetadata(accountPlugin.metadata.ProductName) ?? 'club'
...
export async function ensurePerson (
  ...
  const allowedService = verifyAllowedServices(
    ['tool', 'workspace', 'schedule', 'mail', 'github', 'clubgram'],
    extra,
    false
  )
...
async function getMailboxSecret (
  ...
  verifyAllowedServices(['club-mail'], extra, false)
...
async function exchangeGuestToken (
  ...
  if (tokenObj.account == null) {
    ...
    const oldGuestEmail = '#guest@club.opensrcs.org'
...
async function addClubAssistantSocialId (
  ctx: MeasureContext,
  db: AccountDB,
  branding: Branding | null,
  token: string
  ...
  return await addSocialIdBase(db, account, SocialIdType.CLUB_ASSISTANT, account, true)
}

export async function refreshClubAssistantToken (
  ctx: MeasureContext,
  db: AccountDB,
  branding: Branding | null,
  token: string
  ...
  const assistantSocialId = await db.socialId.findOne({ type: SocialIdType.CLUB_ASSISTANT, personUuid: account })
  if (assistantSocialId == null) {
    throw new PlatformError(
      new Status(Severity.ERROR, platform.status.SocialIdNotFound, {
        value: account,
        type: SocialIdType.CLUB_ASSISTANT
      })
    )
  }
...
export async function releaseSocialId (
  ...
  if (!allowedService) {
    // User should always have at least one club and one "login" social id
    ...
    if (afterRemoval.filter((it) => it.type === SocialIdType.CLUB).length === 0) {
      throw new PlatformError(new Status(Severity.ERROR, platform.status.Forbidden, {}))
    }
...
const loginCapableSocialTypes = [SocialIdType.EMAIL, SocialIdType.CLUB]
...
export type AccountMethods =
  ...
  | 'isReadOnlyGuest'
  | 'addEmailSocialId'
  | 'addClubAssistantSocialId'
  | 'refreshClubAssistantToken'
  | 'releaseSocialId'
  | 'deleteAccount'
...
export function getMethods (hasSignUp: boolean = true): Partial<Record<AccountMethods, AccountMethodHandler>> {
  return {
    ...
    addClubAssistantSocialId: wrap(addClubAssistantSocialId),
    refreshClubAssistantToken: wrap(refreshClubAssistantToken),
