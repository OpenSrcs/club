...
import task, { TaskType, calcRank, makeRank } from '@opensrcs/task'
import time, { ToDo, ToDoPriority } from '@opensrcs/time'
import tracker, { Issue, IssuePriority, IssueStatus, Project } from '@opensrcs/tracker'
...
export class PullRequestSyncManager extends IssueSyncManagerBase implements DocSyncManager {
  ...
  async sync (
    ...
    const needCreateConnectedAtClub = info.addClubLink === true
    ...
    if (existing !== undefined && pullRequestExternal !== undefined && needCreateConnectedAtClub) {
      await this.addClubLink(info, syncResult, existing, pullRequestExternal, container)
    }
    return {
      ...syncResult
    }
