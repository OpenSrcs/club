...
import task, { TaskType, calcRank } from '@opensrcs/task'
import tracker, { Issue, IssuePriority } from '@opensrcs/tracker'
...
export class IssueSyncManager extends IssueSyncManagerBase implements DocSyncManager {
  ...
  async sync (
    ...
    let needCreateConnectedAtClub = info.addClubLink === true
    ...
    if (info.external === undefined && existing !== undefined) {
      ...
      const update: DocumentUpdate<DocSyncInfo> = {
        ...
        githubNumber: issueExternal.number,
        lastModified: new Date(issueExternal.updatedAt).getTime(),
        addClubLink: false, // Do not need, since we create comment on Github about issue is connected.
        current: {
      ...
      needCreateConnectedAtClub = true
    ...
    if (existing !== undefined && issueExternal !== undefined && needCreateConnectedAtClub) {
      await this.addClubLink(info, syncResult, existing, issueExternal, container)
    }
