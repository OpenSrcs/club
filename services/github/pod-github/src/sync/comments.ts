...
import github, { DocSyncInfo, GithubIntegrationRepository, GithubProject } from '@opensrcs/github'
...
export class CommentSyncManager implements DocSyncManager {
  ...
  isClubLinkComment (message: string): boolean {
    return message.includes('<p>Connected to') && message.includes('club&reg;')
  }
  ...
  private async createComment (
    ...
    if ((comment as any).performed_via_github_app !== undefined && this.isClubLinkComment(comment.body)) {
