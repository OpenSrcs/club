...
import { CollaboratorClient } from '@opensrcs/collaborator-client'
import contact, { Person } from '@opensrcs/contact'
import core, {
  AttachedDoc,
  Class,
  Doc,
...
import github, { DocSyncInfo, GithubIntegrationRepository, GithubIssue, GithubProject } from '@opensrcs/github'
import { IntlString } from '@opensrcs/club'
import { LiveQuery } from '@opensrcs/query'
import { getPublicLink } from '@opensrcs/server-guest-resources'
import task, { type Task } from '@opensrcs/task'
import { MarkupNode, MarkupNodeType, areEqualMarkups, markupToJSON, traverseNode } from '@opensrcs/text'
import time, { type ToDo } from '@opensrcs/time'
import tracker, { Issue } from '@opensrcs/tracker'
  ...
  async addClubLink (
    ...
    if (repository !== undefined) {
      ...
      await container.container.octokit.rest.issues.createComment({
        ...
        body: `<p>Connected to <b><a href="${publicLink}">club&reg;: ${(object as Task).identifier}</a></b></p>`,
        headers: {
