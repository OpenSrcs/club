...
import { Branding, generateUuid, PersonUuid, TxOperations, WorkspaceIds, WorkspaceUuid } from '@opensrcs/core'
import { MarkupMarkType, MarkupNode, MarkupNodeType, traverseNode } from '@opensrcs/text'
...
const githubLinkText = process.env.LINK_TEXT ?? 'club&reg;:'

const githubLinkTextOld = 'View in club'

export function hasClubLinkText (text: string): boolean {
  return text.includes(githubLinkText) || text.includes(githubLinkTextOld)
}

export function hasClubLink (href: string, guestLink: string): boolean {
