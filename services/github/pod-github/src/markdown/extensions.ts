import { extensionKit, ServerKit } from '@opensrcs/text'

import { AnyExtension, mergeAttributes, Node } from '@tiptap/core'

export interface SubLinkOptions {
  HTMLAttributes: Record<string, any>
  hasClubText: (text: string) => boolean
  hasClubLink: (href: string) => boolean
}
...
export const SubLink = Node.create<SubLinkOptions>({
  ...
  addOptions () {
    return {
      HTMLAttributes: {},
      hasClubText: (text: string) => false,
      hasClubLink: (href: string) => false
    }
  },
    ...
    return [
      {
        ...
        getAttrs: (el: HTMLElement | string) => {
          if (typeof el !== 'string') {
            if (this.options.hasClubText(el.textContent ?? '')) {
              return null
            }

            const link = el.querySelector('a[href]')
            const href = link?.getAttribute('href') ?? ''
            if (link != null && this.options.hasClubLink(href)) {
              return null
            }
          }
          // no match
