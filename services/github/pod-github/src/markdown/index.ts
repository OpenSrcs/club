...
import { MarkupNode, jsonToMarkup, markupToJSON, htmlToJSON } from '@opensrcs/text'
import { MarkdownParser, storeNodes, storeMarks, MarkdownState } from '@opensrcs/text-markdown'
import { GithubKit } from './extensions'
import { hasClubLink, hasClubLinkText } from '../sync/guest'
...
export function parseMessageMarkdown (
  ...
  const extensions = [
    GithubKit.configure({
      sub: {
        hasClubText: hasClubLinkText,
        hasClubLink: (href) => hasClubLink(href, guestUrl)
      }
    })
  ]
