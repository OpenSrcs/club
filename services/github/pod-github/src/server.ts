...
import { PersonId, BrandingMap, MeasureContext } from '@opensrcs/core'
...
import { decodeToken } from '@opensrcs/server-token'
...
export async function start (ctx: MeasureContext, brandingMap: BrandingMap): Promise<() => Promise<void>> {
  ...
  ctx.info('Running club Github integration', { appId: config.AppID, clientID: config.ClientID })
