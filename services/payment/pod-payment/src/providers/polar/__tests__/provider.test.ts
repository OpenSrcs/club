...
import { AccountClient, SubscriptionType } from '@opensrcs/account-client'
...
describe('PolarProvider', () => {
  ...
  test('createSubscription removes trailing slash from frontUrl to avoid double slashes in URLs', async () => {
    ...
    expect(successUrl).toBe(
      'https://club.opensrcs.org/workbench/tup/setting/setting/billing/subscriptions?payment=success&checkout_id={CHECKOUT_ID}'
    )
    expect(returnUrl).toBe('https://club.opensrcs.org/workbench/tup/setting/setting/billing/subscriptions?payment=canceled')
  ...
  test('createSubscription strips multiple trailing slashes from frontUrl', async () => {
    ...
    const successUrl = mockCreateCheckout.mock.calls[0][1].successUrl
    expect(successUrl).toBe(
      'https://club.opensrcs.org/workbench/ws1/setting/setting/billing/subscriptions?payment=success&checkout_id={CHECKOUT_ID}'
    )
  })
})
