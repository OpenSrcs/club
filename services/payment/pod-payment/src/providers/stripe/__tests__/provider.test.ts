...
import { AccountClient, SubscriptionType, type Subscription } from '@opensrcs/account-client'
...
describe('StripeProvider', () => {
  ...
  test('createSubscription removes trailing slash from frontUrl to avoid double slashes in URLs', async () => {
    const frontUrlWithTrailingSlash = 'https://club.opensrcs.org/'
    ...
    expect(successUrl).toBe(
      'https://club.opensrcs.org/workbench/tup/setting/setting/billing/subscriptions?payment=success&checkout_id={CHECKOUT_SESSION_ID}'
    )
    expect(cancelUrl).toBe('https://club.opensrcs.org/workbench/tup/setting/setting/billing/subscriptions?payment=canceled')
  ...
  test('createSubscription strips multiple trailing slashes from frontUrl', async () => {
    const frontUrlWithMultipleSlashes = 'https://club.opensrcs.org///'
    const provider = new StripeProvider(
      apiKey,
      webhookSecret,
      subscriptionPlans,
    ...
    const successUrl = (stripeClient.createCheckout as jest.Mock).mock.calls[0][1].successUrl
    expect(successUrl).toBe(
      'https://club.opensrcs.org/workbench/ws1/setting/setting/billing/subscriptions?payment=success&checkout_id={CHECKOUT_SESSION_ID}'
    )
  })
