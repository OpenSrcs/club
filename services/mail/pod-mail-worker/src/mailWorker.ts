...
import { MeasureContext, WorkspaceUuid, Doc, TxCUD, Tx } from '@opensrcs/core'
import {
  toMessageEvent,
  isNewChannelTx,
  markdownToHtml,
...
import { ConsumerHandle, PlatformQueue, QueueTopic } from '@opensrcs/server-core'
...
export class MailWorker {
  ...
  async handleNewMessage (workspaceUuid: WorkspaceUuid, message: CreateMessageEvent): Promise<void> {
    try {
      ...
      try {
        ...
        if (channel === undefined || !this.isClubMailChannel(channel)) {
          return
        }
  ...
  private async sendMessageAsEmail (
    ...
    try {
      ...
      const mailMessage: MailMessage = {
        ...
        html,
        text,
        headers: getMailHeadersRecord(ClubMessageType, message._id, email)
      }
  ...
  isClubMailChannel (channel: Card): boolean {
