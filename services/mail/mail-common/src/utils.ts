...
import { BlobMetadata, MeasureContext } from '@opensrcs/core'
import {
  Attachment,
  EmailContact,
  EmailMessage,
  ClubMailHeader,
  ClubMessageIdHeader,
  ClubMessageTypeHeader,
  MailHeader
} from './types'
import { MessageExtra, MessageID } from '@opensrcs/communication-types'
import { CreateMessageEvent } from '@opensrcs/communication-sdk-types'
import { generateMessageId } from '@opensrcs/communication-shared'
...
export function getMailHeaders (messageType: string, messageId?: string | undefined): string[] {
  const headers = [`${ClubMailHeader}: true\n`, `${ClubMessageTypeHeader}: ${messageType}\n`]
  if (messageId !== undefined) {
    headers.push(`${ClubMessageIdHeader}: ${messageId}\n`)
  }
  return headers
}
...
export function getMailHeadersRecord (
  ...
  const headers: Record<string, string> = {
    [ClubMailHeader]: 'true',
    [ClubMessageTypeHeader]: messageType,
    [MailHeader.Id]: getEmailMessageIdFromClubId(messageId, email)
  }
  if (messageId !== undefined) {
    headers[ClubMessageIdHeader] = messageId
  }
  return headers
}

export function isClubMessage (headers: string[]): boolean {
  return headers.some(
    (header) =>
      header.startsWith(ClubMailHeader) ||
      header.startsWith(ClubMessageIdHeader) ||
      header.startsWith(ClubMessageTypeHeader)
  )
}
...
export function getEmailMessageIdFromClubId (clubId: string | undefined, email: string): string {
  const domain = getDomainFromEmail(email)
  const id = clubId ?? generateMessageId()
  return `<${id}@${domain}>`
}

export function getClubIdFromEmailMessageId (messageId: string, email: string): MessageID | undefined {
  const domain = getDomainFromEmail(email)
...
export function generateNewEmailId (email: string): string {
  return getEmailMessageIdFromClubId(generateMessageId(), email)
}

export function isClubEmailMessageId (messageId: string, email: string): boolean {
  return getClubIdFromEmailMessageId(messageId, email) !== undefined
}
