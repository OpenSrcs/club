...
import { WorkspaceLoginInfo } from '@opensrcs/account-client'
import { type Card } from '@opensrcs/card'
import { MessageID, MessageType } from '@opensrcs/communication-types'
...
import { type KeyValueClient } from '@opensrcs/kvs-client'
...
import { getBlobMetadata, getClubIdFromEmailMessageId, getMdContent, MessageTimeShift } from './utils'
...
async function createMailMessage (
  ...
  const messageId = getClubIdFromEmailMessageId(data.mailId, data.from) ?? generateMessageId()
