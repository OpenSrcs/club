...
import { type MeasureContext, TxOperations } from '@opensrcs/core'
import {
  createMessages,
  parseEmailHeader,
...
import { AccountClient, isWorkspaceLoginInfo, WorkspaceLoginInfo } from '@opensrcs/account-client'
...
export class MessageManagerV2 implements IMessageManager {
  ...
  async saveMessage (
    ...
    if (isClubMessage(message.data.payload)) {
      this.ctx.info('Skipping club message', { mailId: message.data.id, me })
      return
    }
...
export function isClubMessage (payload: gmail_v1.Schema$MessagePart | undefined): boolean {
  const clubHeader = getHeaderValue(payload, MailHeader.ClubSent)
  if (clubHeader !== undefined) {
    return true
  }
  const clubMessage = getHeaderValue(payload, MailHeader.ClubMessageType)
