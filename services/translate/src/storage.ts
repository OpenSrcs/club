...
import { MeasureContext, systemAccountUuid, WorkspaceUuid } from '@opensrcs/core'
import { type ClublakeWorkspaceClient, type JsonPatch, getWorkspaceClient } from '@opensrcs/clublake-client'
...
export class Storage {
  ...
  private getClient (ws: WorkspaceUuid): ClublakeWorkspaceClient {
    const token = generateToken(systemAccountUuid, ws, undefined, config.Secret)
    return getWorkspaceClient(config.ClublakeUrl, ws, token)
  }
