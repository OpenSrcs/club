...
The Export Service provides asynchronous data export functionality for club workspaces. It allows workspace owners and administrators to export documents and their associated data in multiple formats (JSON, CSV, Unified).
...
- Converts club documents to standardized formats
...
The converter transforms club documents into a unified format that:
...
```bash
docker build -t opensrcs/export .
...
```bash
rush build --to @opensrcs/pod-export
```
...
ACCOUNTS_URL="http://127.0.0.1:3000" SECRET="secret" DB_URL=postgresql://root@club.local:26257/defaultdb?sslmode=disable SERVICE_ID="export" STORAGE_CONFIG="datalake|http://club.local:4030" rushx run-local
```
...
For issues and questions, see the main OpenSrcs Club repository.
