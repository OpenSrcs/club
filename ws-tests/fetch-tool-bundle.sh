...
# Extract the version of opensrcs/transactor from the docker-compose file
VERSION=$(grep "image: opensrcs/transactor:" "$DOCKER_COMPOSE_PATH" | head -1 | sed -E 's/.*image: opensrcs\/transactor:([^[:space:]]+).*/\1/')
...
echo "Fetching opensrcs/tool:$VERSION..."

# Pull the tool image with the same version
docker pull "opensrcs/tool:$VERSION"

# Create a temporary container from the image
CONTAINER_ID=$(docker container create "opensrcs/tool:$VERSION")
