...
# Extract opensrcs services and add them to the override file with fixed versions
grep -B 1 "image: opensrcs/" $COMPOSE_FILE | grep -v "\-\-" | grep -v "image:" | sed 's/:$//g' | while read -r service; do
    service=$(echo $service | tr -d ' ')
    if [ -n "$service" ]; then
        echo "  $service:" >> $OVERRIDE_FILE
        
        # Get the image name
        image=$(grep -A 1 "$service:" $COMPOSE_FILE | grep "image: opensrcs/" | awk '{print $2}')
        pod_name=$(echo $image | sed 's/opensrcs\///')
        
        echo "    image: opensrcs/$pod_name:$VERSION" >> $OVERRIDE_FILE
        ...
        if [ "$pod_name" == "workspace" ]; then
          echo "    environment:" >> $OVERRIDE_FILE
          echo "      - INIT_WORKSPACE=club" >> $OVERRIDE_FILE 
        fi
    fi
done
...
echo "Created $OVERRIDE_FILE with fixed version $VERSION for all opensrcs services"
