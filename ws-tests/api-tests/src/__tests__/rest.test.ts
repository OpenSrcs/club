...
import platform from '@opensrcs/club'
import { RPCHandler } from '@opensrcs/rpc'
import { type AccountClient, getClient as getAccountClient } from '@opensrcs/account-client'
import chunter from '@opensrcs/chunter'
import contact, { ensureEmployee, type SocialIdentityRef, type Person } from '@opensrcs/contact'
...
describe('rest-api-server', () => {
  ...
  beforeAll(async () => {
    serverConfig = await loadServerConfig('http://club.local:8083')
    const config = serverConfig

    apiWorkspace1 = await getWorkspaceToken(
      'http://club.local:8083',
      {
        email: 'user1',
        password: '1234',
        workspace: wsName
