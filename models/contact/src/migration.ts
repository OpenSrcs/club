...
import core, { getAccountsFromTxes, getSocialIdBySocialKey, getSocialKeyByOldEmail } from '@opensrcs/model-core'
import { DOMAIN_VIEW } from '@opensrcs/model-view'
import card, { type Card, DOMAIN_CARD } from '@opensrcs/card'
...
async function migrateMergedAccounts (client: MigrationClient): Promise<void> {
  ...
  for (const account of personAccounts) {
    ...
    if (account.email === '' || account.email === 'club.ai.bot@club.opensrcs.org') {
      continue
    }
