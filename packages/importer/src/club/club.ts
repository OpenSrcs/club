...
import { type Attachment } from '@opensrcs/attachment'
import card from '@opensrcs/card'
import contact, { type Employee, type Person } from '@opensrcs/contact'
import documents, {
  type ControlledDocument,
  type DocumentCategory,
  type DocumentMeta,
  DocumentState
} from '@opensrcs/controlled-documents'
import {
  type AccountUuid,
  generateId,
  type PersonId,
...
import document, { type Document } from '@opensrcs/document'
import core from '@opensrcs/model-core'
import tracker, { type Issue, type Project } from '@opensrcs/tracker'
...
import { ClubMarkdownPreprocessor, type AttachmentMetadata } from './preprocessor'
...
export interface ClubIssueHeader {
...
export interface ClubSpaceSettings {
...
export interface ClubProjectSettings extends ClubSpaceSettings {
...
export interface ClubTeamspaceSettings extends ClubSpaceSettings {
  class: 'document:class:Teamspace'
}

export interface ClubDocumentHeader {
  class: 'document:class:Document'
  title: string
}

export interface ClubWorkspaceSettings {
  projectTypes?: Array<{
    name: string
    taskTypes?: Array<{
      name: string
...
export interface ClubChangeControlHeader {
  description?: string
  reason?: string
  impact?: string
}

export interface ClubControlledDocumentHeader {
  ...
  approvers?: string[]
  coAuthors?: string[]
  changeControl?: ClubChangeControlHeader
}

export interface ClubDocumentTemplateHeader {
...
export interface ClubOrgSpaceSettings extends ClubSpaceSettings {
...
export class ClubFormatImporter {
  ...
  async importFolder (folderPath: string): Promise<void> {
    ...
    const preprocessor = new ClubMarkdownPreprocessor(
  ...
  private async processImportFolder (folderPath: string): Promise<ImportWorkspace> {
    ...
    if (fs.existsSync(wsSettingsPath)) {
      ...
      const wsSettings = yaml.load(wsSettingsFile) as ClubWorkspaceSettings
    ...
    for (const yamlFile of yamlFiles) {
      ...
      try {
        ...
        const spaceConfig = this.parser.readYaml(yamlPath) as ClubSpaceSettings
        ...
        switch (spaceConfig.class) {
          case tracker.class.Project: {
            const project = await this.processProject(spaceConfig as ClubProjectSettings)
          ...
          case document.class.Teamspace: {
            const teamspace = await this.processTeamspace(spaceConfig as ClubTeamspaceSettings)
          ...
          case documents.class.OrgSpace: {
            const orgSpace = await this.processOrgSpace(spaceConfig as ClubOrgSpaceSettings)
  ...
  private async processIssuesRecursively (
    ...
    for (const issueFile of issueFiles) {
      ...
      const issueHeader = this.parser.readYamlHeader(issuePath) as ClubIssueHeader
  ...
  private async processDocumentsRecursively (
    ...
    for (const docFile of docFiles) {
      ...
      const docHeader = this.parser.readYamlHeader(docPath) as ClubDocumentHeader
  ...
  private async processControlledDocumentsRecursively (
    ...
    for (const docFile of docFiles) {
      const docPath = path.join(currentPath, docFile)
      const docHeader = this.parser.readYamlHeader(docPath) as ClubControlledDocumentHeader | ClubDocumentTemplateHeader
      ...
      if (docHeader.class === documents.class.ControlledDocument) {
        const doc = await this.processControlledDocument(
          docHeader as ClubControlledDocumentHeader,
          docPath,
        ...
        const template = await this.processControlledDocumentTemplate(
          docHeader as ClubDocumentTemplateHeader,
          docPath,
          this.metadataRegistry.getRef(docPath) as Ref<ControlledDocument>,
          documentMetaId
        )
  ...
  private processComments (currentPath: string, comments: ClubComment[] = []): Promise<ImportComment[]> {
    return Promise.all(
  ...
  private processProjectTypes (wsHeader: ClubWorkspaceSettings): ImportProjectType[] {
    return (
  ...
  private async processProject (data: ClubProjectSettings): Promise<ImportProject> {
    return {
      class: tracker.class.Project,
  ...
  private async processTeamspace (data: ClubTeamspaceSettings): Promise<ImportTeamspace> {
    return {
      class: document.class.Teamspace,
      title: data.title,
      private: data.private ?? false,
  ...
  private async processOrgSpace (data: ClubOrgSpaceSettings): Promise<ImportOrgSpace> {
    return {
      class: documents.class.OrgSpace,
      title: data.title,
      private: data.private ?? false,
  ...
  private async processControlledDocument (
    header: ClubControlledDocumentHeader,
    docPath: string,
  ...
  private async processControlledDocumentTemplate (
    header: ClubDocumentTemplateHeader,
    docPath: string,
    id: Ref<ControlledDocument>,
    metaId: Ref<DocumentMeta>
