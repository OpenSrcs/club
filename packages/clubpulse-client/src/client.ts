...
export class ClubpulseClient implements Disposable {
          ...
          if ('message' in msg) {
            ...
            if (id !== undefined && this.pending.has(id)) {
              ...
              if (pending !== undefined) {
                ...
                if ('error' in msg) {
                  if (typeof msg.error === 'string' && ClubpulseClient.isConnectionLikeError(msg.error)) {
  ...
  static async connect (url: string | URL): Promise<ClubpulseClient> {
    const client = new ClubpulseClient(url)
