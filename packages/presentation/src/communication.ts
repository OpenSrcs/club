...
//
import { closeLiveQueries, initLiveQueries, refreshLiveQueries } from '@opensrcs/communication-client-query'
import {
...
} from '@opensrcs/communication-sdk-types'
import {
  type AccountUuid,
  type CardID,
  type CardType,
...
import { addNotification, NotificationSeverity, languageStore } from '@opensrcs/ui'
import { getMetadata, translate } from '@opensrcs/club'
...
} from '@opensrcs/communication-client-query'
...
export async function setCommunicationClient (platformClient: PlatformClient): Promise<void> {
  ...
  const clublakeUrl = getMetadata(presentation.metadata.ClublakeUrl) ?? ''
  const clublake = getClublakeClient(clublakeUrl, getCurrentWorkspaceUuid(), token)

  initLiveQueries(_client, clublake, onDestroy)
  client = _client
...
class Client {
  ...
  private getSocialId (): SocialID {
    ...
    const clubSocialId = me.fullSocialIds.find((it) => it.type === SocialIdType.CLUB && it.verifiedOn !== undefined)
    const id = clubSocialId?._id ?? me.primarySocialId
    if (id == null || id === '') {
