...
import { ClubpulseClient } from '@opensrcs/clubpulse-client'
import { getMetadata } from '@opensrcs/club'
import presentation from './plugin'

let currentWorkspaceUuid: string | undefined
let currentToken: string | undefined
let promise: Promise<ClubpulseClient | undefined> | undefined

export async function createPulseClient (): Promise<ClubpulseClient | undefined> {
  ...
  promise = new Promise((resolve) => {
    ClubpulseClient.connect(`${pulseUrl}?token=${token}`)
      .then(resolve)
      .catch(() => {
        resolve(undefined)
      })
