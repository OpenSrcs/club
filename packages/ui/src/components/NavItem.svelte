...
-->
<script lang="ts">
  import type { Asset, IntlString } from '@opensrcs/club'
...
<button
  class="clubNavItem-container line-height-auto {type} {type === 'type-anchor-link'
    ? 'font-regular-12'
    : 'font-regular-14'}"
  class:selected
  class:bold
...
>
  ...
  {#if visibleIcon || (type === 'type-tag' && color)}
    <div class="clubNavItem-icon relative" class:withBackground class:w-auto={iconSize === 'x-small'}>
      {#if $$slots.icon}
        <slot name="icon" />
      {:else if type !== 'type-tag' && visibleIcon}
        <Icon icon={visibleIcon} size={iconSize} {iconProps} />
      {:else if type === 'type-tag'}
        <div style:background-color={color} class="clubNavItem-icon__tag" />
      {/if}
    </div>
  {/if}
  <span
    ...
    class="{description ? 'clubNavItem-wideLabel' : 'clubNavItem-label'} overflow-label"
    class:flex-grow={!(type === 'type-anchor-link')}
    style:color={type === 'type-tag' && selected ? color : null}
  >
    {#if description}
      <span class="clubNavItem-label font-medium-12 line-height-auto mr-0-5">
  ...
  {#if showMenu || $$slots.actions}
    <div class="clubNavItem-actions">
      <slot name="actions" />
    </div>
  {/if}
  {#if count !== null}
    <span class="clubNavItem-count font-bold-12">
      {count}
    </span>
  {/if}
  <slot name="notify" />
  ...
  .clubNavItem-container {
    ...
    &.type-link {
      ...
      &.selected {
        ...
        .clubNavItem-label:not(.description) {
          color: var(--global-accent-TextColor);
        }
      }
    }
    ...
    &.type-anchor-link {
      ...
      &.selected .clubNavItem-icon,
      &.selected .clubNavItem-label {
        color: var(--global-primary-TextColor);
      }
    }
    &.indent {
    ...
    &:hover .clubNavItem-chevron:enabled {
      color: var(--global-secondary-TextColor);
      background-color: var(--button-tertiary-hover-BackgroundColor);
    }

    &:not(.noActions):hover .clubNavItem-actions,
    &:not(.noActions).showMenu .clubNavItem-actions {
      display: flex;
    }
    &.disabled {
      cursor: not-allowed;
