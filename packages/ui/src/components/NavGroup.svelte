...
-->
<script lang="ts">
  import type { Doc, Ref } from '@opensrcs/core'
...
>
  ...
  >
    {#if isFold && !empty}
      <button class="clubNavGroup-header__chevron" class:collapsed={!isOpen} on:click={toggle}>
        <IconDown size={'small'} />
      </button>
    {/if}
    <!-- svelte-ignore a11y-no-static-element-interactions -->
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <div class="clubNavGroup-header__label font-medium-12" on:click={handleMenuClicked}>
      {#if visibleIcon}
        <div class="clubNavGroup-header__icon" class:folder={folderIcon}>
    ...
    {#if tools || $$slots.tools}
      <div class="clubNavGroup-header__tools">
        {#if tools}
          <Component
            is={tools}
            props={{
    ...
    {/if}
    {#if $$slots.actions}
      <div class="clubNavGroup-header__actions">
        <slot name="actions" />
      </div>
    {/if}
    {#if selected && type === 'nested-selectable'}
      <div class="clubNavGroup-header__arrow"><IconOpenedArrow size={'small'} /></div>
    {/if}
  </button>
  {#if !empty}
    <div {id} class="clubNavGroup-content">
