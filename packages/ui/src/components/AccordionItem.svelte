...
<div class="clubAccordionItem-container {kind}" class:nested>
  <button
    class="clubAccordionItem-header {kind} {size}"
    class:bottomSpace
    class:nested
    class:disabled
  ...
  >
    <div
      class="clubAccordionItem-header__label-wrapper {size === 'large' ? 'heading-medium-16' : 'font-medium-12'}"
      class:withIcon={size === 'medium' && icon !== undefined}
    >
      {#if size === 'large' && !disabled}
        <div class="clubAccordionItem-header__chevron">
      ...
      <div
        class="clubAccordionItem-header__label"
      ...
      {#if counter !== false && ($$slots.counter || typeof counter === 'number')}
        <span class="clubAccordionItem-header__separator">•</span>
        <span class="clubAccordionItem-header__counter">
      ...
      {#if duration !== false && ($$slots.duration || duration !== 0)}
        <span class="clubAccordionItem-header__separator">•</span>
        <span class="clubAccordionItem-header__duration">
          {#if typeof duration === 'number' && durationLabel !== ''}{durationLabel}{/if}
          <slot name="duration" />
        </span>
      {/if}
    </div>
    <div class="clubAccordionItem-header__tools">
      <slot name="actions" />
    </div>
  </button>
  <div class="clubAccordionItem-content" style:align-items={contentAlign}>
    <slot />
  </div>
</div>
