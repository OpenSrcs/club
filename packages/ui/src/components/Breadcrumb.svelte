...
<button class="clubBreadcrumb-container {size}" class:current={isCurrent} on:click>
  {#if size === 'large' && icon}
    <div
      class="clubBreadcrumb-avatar"
  ...
  <span
    class="{size === 'large'
      ? 'heading-medium-16'
      : 'font-regular-14'} line-height-auto clubBreadcrumb-label overflow-label"
  >
...
<style lang="scss">
  .clubBreadcrumb-container {
    ...
    .clubBreadcrumb-label {
      padding: var(--spacing-0_5) 0;
      color: var(--global-secondary-TextColor);
    }
    &.current .clubBreadcrumb-label {
      font-weight: 700;
    }
    &:not(.current) {
      cursor: pointer;
