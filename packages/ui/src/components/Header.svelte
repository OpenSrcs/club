...
>
  {#if _doubleRow}
    <div class="clubHeader-row">
      {#if allowFullsize}
        ...
        />
        <div class="clubHeader-divider no-print" />
      {/if}
      {#if $$slots.beforeTitle && !hideBefore}
        <div class="clubHeader-buttonsGroup before mr-2 no-print" class:freezeBefore>
      ...
      >
        {#if $$slots.description && !hideDescription}
          <div class="clubHeader-titleGroup"><slot /></div>
          <div class="clubHeader-titleGroup"><slot name="description" /></div>
        {:else}
          <slot />
        {/if}
      </div>
      ...
      {#if $$slots.actions && !hideActions && (adaptive === 'freezeActions' || adaptive === 'doubleRow' || adaptive === 'autoExtra')}
        <div class="clubHeader-buttonsGroup actions no-print">
          <slot name="actions" {doubleRow} />
        </div>
      {/if}
      {#if closeButton}
        {#if type !== 'type-popup'}<div class="clubHeader-divider no-print" />{/if}
        {#if closeOnEscape}
          <div class="clubHotKey-item no-print">Esc</div>
        {/if}
        <ButtonIcon icon={IconClose} kind={'tertiary'} size={'small'} noPrint on:click={() => dispatch('close')} />
      {/if}
    </div>
    <!-- <div class="clubHeader-row__divider" /> -->
    <div class="clubHeader-row no-print" class:between={$$slots.search} class:reverse={!$$slots.search}>
      {#if $$slots.presence && !hidePresence}
        <div class="clubHeader-buttonsGroup presence no-print">
          <slot name="presence" {doubleRow} />
        </div>
      {/if}
      {#if $$slots.search}
        <div class="clubHeader-buttonsGroup search">
          <slot name="search" {doubleRow} />
        </div>
      {/if}
      {#if $$slots.extra && !hideExtra && (adaptive === 'doubleRow' || adaptive === 'autoExtra')}
        <div
          class="clubHeader-buttonsGroup extra"
      ...
      {/if}
      {#if $$slots.actions && !hideActions && !(adaptive === 'freezeActions' || adaptive === 'doubleRow' || adaptive === 'autoExtra')}
        <div class="clubHeader-buttonsGroup actions flex-shrink">
          <slot name="actions" {doubleRow} />
        </div>
      {/if}
    </div>
  {:else}
    ...
    {/if}
    {#if $$slots.beforeTitle && !hideBefore}
      <div class="clubHeader-buttonsGroup before mr-2 no-print">
        <slot name="beforeTitle" {doubleRow} />
      </div>
    {/if}
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    ...
    {#if $$slots.search && !hideSearch}
      <div class="clubHeader-buttonsGroup search no-print">
        <slot name="search" {doubleRow} />
      </div>
      {#if $$slots.actions && !hideActions}<div class="clubHeader-divider no-print" />{/if}
    {/if}
