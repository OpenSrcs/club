...
import { MeasureContext, metricsAggregate, WorkspaceUuid } from '@opensrcs/core'
import { getCPUInfo, getMemoryInfo, StorageConfiguration } from '@opensrcs/server-core'
import { buildStorageFromConfig, storageConfigFromEnv } from '@opensrcs/server-storage'
import { decodeToken, TokenError } from '@opensrcs/server-token'
...
export async function createServer (ctx: MeasureContext, config: Config): Promise<{ app: Express, close: () => void }> {
  ...
  app.get('/', (_req, res) => {
    res.send(`
      club&reg; Preview&trade; <a href="https://club.opensrcs.org">https://club.opensrcs.org</a>
