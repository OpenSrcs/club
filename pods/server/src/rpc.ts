import { getClient as getAccountClientRaw, type AccountClient } from '@opensrcs/account-client'
import contact, {
  AvatarType,
  combineName,
  type Person,
...
import { rpcJSONReplacer, type RateLimitInfo } from '@opensrcs/rpc'
import type { ClientSessionCtx, ConnectionSocket, Session, SessionManager } from '@opensrcs/server-core'
import { setApiTokenRevocationChecker, verifyToken, type Token } from '@opensrcs/server-token'
...
import platform, { PlatformError, unknownError } from '@opensrcs/club'
...
export function registerRPC (app: Express, sessions: SessionManager, ctx: MeasureContext, accountsUrl: string): void {
  ...
  // To use in non-js (rust) clients that can't link to @opensrcs/core
