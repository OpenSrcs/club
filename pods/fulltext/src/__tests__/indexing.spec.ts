...
import { createPlatformQueue, parseQueueConfig } from '@opensrcs/kafka'
import {
...
import { decodeToken, generateToken } from '@opensrcs/server-token'
...
class TestQueue {
  ...
  async start (): Promise<void> {
    ...
    this.mgr = new TestWorkspaceManager(this.ctx, model, {
      ...
      serverSecret: 'secret',
      dbURL: dbUrl,
      clublakeUrl: 'http://localhost:8096',
      config: dbConfig,
