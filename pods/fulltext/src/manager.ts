...
import core, { Hierarchy, systemAccountUuid, TxProcessor, versionToString } from '@opensrcs/core'
import { getAccountClient, getTransactorEndpoint } from '@opensrcs/server-client'
import {
...
import { type QueueSourced, type FulltextDBConfiguration } from '@opensrcs/server-indexer'
import { generateToken } from '@opensrcs/server-token'
import { type Event } from '@opensrcs/communication-sdk-types'
  ...
  async createIndexer (
    ...
    while (true) {
      ...
      return await WorkspaceIndexer.create(
        ...
        this.contentAdapter,
        getClublakeClient(this.opt.clublakeUrl, workspace, token ?? ''),
