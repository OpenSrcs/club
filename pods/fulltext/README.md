...
- **`CLUBLAKE_URL`** - Clublake service URL for communication indexing (default: empty string)
...
- **`COMMUNICATION_API_ENABLED`** - Enable communication API for indexing messages (default: disabled)
  ...
  - When disabled, communication indexing is skipped even if `CLUBLAKE_URL` is provided
...
- Use Clublake service to fetch message groups and messages
- Require `CLUBLAKE_URL` to be set

When disabled (default), communication indexing is skipped, preventing errors if Clublake is not available.
