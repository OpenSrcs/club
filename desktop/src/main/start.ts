...
import { generateId } from '@opensrcs/core'
import { DownloadItem } from '@opensrcs/desktop-downloads'
...
function runTheApp (): void {
  ...
  const sessionPartition = !isDev ? 'persist:club' : 'persist:club_dev'
  ...
  console.log('Running club', process.env.MODEL_VERSION, process.env.VERSION, isMac, isDev, process.env.NODE_ENV)
  ...
  ipcMain.on(IpcMessage.SetCombinedConfig, (_event: any, config: Config) => {
    ...
    const updatesUrl = process.env.DESKTOP_UPDATES_URL ?? config.DESKTOP_UPDATES_URL ?? 'https://dist.opensrcs.org'
    // NOTE: env format is: default_value;key1:value1;key2:value2...
    const updatesChannels = (process.env.DESKTOP_UPDATES_CHANNEL ?? config.DESKTOP_UPDATES_CHANNELS ?? config.DESKTOP_UPDATES_CHANNEL ?? 'club').split(';').map(c => c.trim().split(':'))
    ...
    const updatesChannel = updateChannelsMap[updatesChannelKey] ?? updateChannelsMap.default ?? 'club'
