...
export function getWorkspaceClient (baseUrl: string, workspace: WorkspaceUuid, token: string): ClublakeWorkspaceClient {
  const client = new Client(baseUrl, token)
  return new WorkspaceClient(client, workspace)
}

export function getClient (baseUrl: string, token: string): ClublakeClient {
  return new Client(baseUrl, token)
}

class WorkspaceClient implements ClublakeWorkspaceClient {
  constructor (
    private readonly client: ClublakeClient,
  ...
  public async getJson<T>(key: string, retryOptions?: RetryOptions): Promise<ClubResponse<T>> {
...
class Client implements ClublakeClient {
  constructor (
  ...
  private applyHeaders (h: Headers, headers?: ClubHeaders): void {
    if (headers != null) for (const [k, v] of Object.entries(headers)) h.set(`club-header-${k}`, v)
  }

  private applyMeta (h: Headers, meta?: ClubMeta): void {
    if (meta != null) for (const [k, v] of Object.entries(meta)) h.set(`club-meta-${k}`, v)
  }
  ...
  public async head (workspace: string, key: string, retryOptions?: RetryOptions): Promise<ClubResponse<void>> {
  ...
  public async put (
    ...
    if (mergeStrategy != null) {
      h.set('club-Merge-Strategy', mergeStrategy)
    }
  ...
  public async delete (workspace: string, key: string, retryOptions?: RetryOptions): Promise<ClubResponse<void>> {
    const res = await fetchSafe(
      this.objectUrl(workspace, key),
      {
        method: 'DELETE',
