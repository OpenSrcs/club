...
describe('Storage Client Integration Tests', () => {
  ...
  describe('Clublake Storage Integration', () => {
    let storage: FileStorage
    ...
    it('should complete full file lifecycle with Clublake storage', async () => {
      const token = 'test-token'
      const workspace = 'clublake-workspace'
      const uuid = 'clublake-file-uuid'
      const filename = 'clublake-test.txt'
      const file = new File(['Clublake test content'], filename, { type: 'text/plain' })
      ...
      expect(mockUploadXhr).toHaveBeenCalledWith(
        {
          url: `https://clublake.example.com/api/${workspace}/${uuid}`,
          method: 'PUT',
          headers: {
      ...
      // Generate file URL
      const fileUrl = storage.getFileUrl(workspace, uuid, filename)
      expect(fileUrl).toBe(`https://clublake.example.com/api/${workspace}/${uuid}`)

      // Get file metadata (should return empty object for Clublake storage)
      const meta = await storage.getFileMeta(token, workspace, uuid)
      expect(meta).toEqual({})
  ...
  describe('Error Handling Integration', () => {
    it('should handle upload failures across all storage types', async () => {
      const configs: Array<{ name: string, config: FileStorageConfig }> = [
        ...
        {
          name: 'Clublake',
          config: { uploadUrl: 'https://front.example.com', clublakeUrl: 'https://clublake.example.com' }
        }
      ]
  ...
  describe('Configuration Priority Integration', () => {
    it('should respect storage priority in real usage scenarios', async () => {
      // Test that Datalake takes priority over Clublake
      const config: FileStorageConfig = {
      ...
      // Should use Datalake form-data endpoint, not Clublake PUT endpoint
      expect(mockUploadXhr).toHaveBeenCalledWith(
        expect.objectContaining({
          url: `https://datalake.example.com/upload/form-data/${encodeURIComponent(workspace)}`,
          method: 'POST' // Datalake uses POST for form-data, Clublake uses PUT
        }),
        undefined
      )
    })
