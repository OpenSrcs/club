...
describe('ClublakeStorage', () => {
  let storage: ClublakeStorage
  const baseUrl = 'https://clublake.example.com'

  beforeEach(() => {
    storage = new ClublakeStorage(baseUrl)
    jest.clearAllMocks()
    mockFetch.mockClear()
  ...
  describe('getFileUrl', () => {
    ...
    it('should handle base URL with trailing slash', () => {
      const storageWithSlash = new ClublakeStorage('https://clublake.example.com/')
      const workspace = 'test-workspace'
      const file = 'file-123'

      const url = storageWithSlash.getFileUrl(workspace, file)

      expect(url).toBe('https://clublake.example.com/api/test-workspace/file-123')
    })

    it('should handle base URL without trailing slash', () => {
      const storageWithoutSlash = new ClublakeStorage('https://clublake.example.com')
      const workspace = 'test-workspace'
      const file = 'file-123'
  ...
  describe('integration scenarios', () => {
    it('should handle complete file lifecycle', async () => {
      ...
      // Get file meta (should return empty object for ClublakeStorage)
