# OpenSrcs Club API Client

A TypeScript client library for interacting with the OpenSrcs Club API.
...
```bash
npm install @opensrcs/api-client
```

## WebSocket Client vs REST Client

The api client package provides two main client variants: a WebSocket client and a REST client. The WebSocket client holds persistent connection to the OpenSrcs Club API. The REST client uses standard HTTP requests to perform operations.

### WebSocket Client

```ts
import { connect } from '@opensrcs/api-client'

// Connect to club
const client = await connect('https://club.opensrcs.org', {
...
```ts
import { connectRest } from '@opensrcs/api-client'

// Connect to club
const client = await connectRest('https://club.opensrcs.org', {
  email: 'johndoe@example.com',
  password: 'password',
  workspace: 'my-workspace'
})
...
- `url`: URL of the club instance, for club Cloud use `https://club.opensrcs.org`
- `options`: Connection options
  - `workspace`: Name of the workspace to connect to, the workspace name can be found in the URL of the workspace: `https://club.opensrcs.org/workbench/<workspace-name>`
...
The client provides a set of methods for interacting with the OpenSrcs Club API. This section describes the main methods available in the client.
...
```ts
import contact, { AvatarType } from '@opensrcs/contact'
