...
export function pickPrimarySocialId (socialIds: SocialId[]): SocialId {
  ...
  const clubSocialIds = activeSocialIds.filter((si) => si.type === SocialIdType.CLUB)

  return clubSocialIds[0] ?? activeSocialIds[0]
}
...
export { platformNow, platformNowDiff } from '@opensrcs/measurements'
