...
impl<'de> serde::Deserialize<'de> for PatchOperation {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if let Ok(op) = op {
            ...
            serde_json::from_value::<ClubPatchOperation>(value)
                .map_err(|patch_error| {
                    serde::de::Error::custom(format!(
                        "Failed to deserialize as StandardPatchOperation: {}. Also failed to deserialize as ClubPatchOperation: {}",
                        standard_error, patch_error
                    ))
                })
                .map(Self::Club)
        }
...
impl From<json_patch::PatchError> for ClubPatchError {
    fn from(err: json_patch::PatchError) -> Self {
        ClubPatchError::PatchError(err.to_string())
    }
}

pub fn apply(doc: &mut Value, patches: &[PatchOperation]) -> Result<(), ClubPatchError> {
    for patch in patches {
        if let Some(op) = match patch {
            PatchOperation::Club(patch_op) => match patch_op {
                ClubPatchOperation::Add(op) => add(doc, &op.path, &op.value, op.safe),
                ClubPatchOperation::Inc(op) => inc(doc, &op.path, &op.value, op.safe),
                ClubPatchOperation::Remove(op) => remove(doc, &op.path, op.safe),
            },
...
fn add(
    ...
    value: &Value,
    safe: bool,
) -> Result<Option<StandardPatchOperation>, ClubPatchError> {
...
fn inc(
    ...
    match (target, value.as_number()) {
        ...
        (Some(_), None) => Err(ClubPatchError::InvalidNumber),
        ...
        (Some(_), Some(_)) => Err(ClubPatchError::InvalidNumber),
        (None, None) => Err(ClubPatchError::InvalidNumber),
    }
}
...
fn add_json_numbers(a: &Number, b: &Number) -> Result<Number, ClubPatchError> {
    a.as_i64()
        ...
        .ok_or(ClubPatchError::InvalidNumber)
}
...
mod tests {
    ...
    fn test_add(
        ...
        value: Value,
        safe: bool,
        expected: Result<Option<json_patch::PatchOperation>, ClubPatchError>,
    ) {
    ...
    fn test_inc_non_number_field() {
        test_inc(
            ...
            json!(1),
            false,
            Err(ClubPatchError::InvalidNumber),
        );
    }
    ...
    fn test_patch() {
        ...
        let patches = vec![
            PatchOperation::Club(ClubPatchOperation::Add(AddOperationExt {
                path: PointerBuf::from_tokens(["a"]),
                value: json!([]),
                safe: false,
            })),
            PatchOperation::Club(ClubPatchOperation::Inc(IncOperationExt {
                path: PointerBuf::from_tokens(["a", "0"]),
                value: json!(1),
                safe: true,
            })),
    ...
    fn test_patch_err() {
        ...
        assert_eq!(
            res,
            Err(ClubPatchError::PatchError(String::from(
