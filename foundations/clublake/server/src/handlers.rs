...
pub struct Headers {
    ...
    pub club_headers: Vec<(String, String)>,
...
async fn extract_headers(request: &mut ServiceRequest) -> HandlerResult<(Headers, MergeStrategy)> {
    ...
    let mut club_headers = Vec::new();
    for (key, value) in request.headers().iter() {
        if let Some(header) = key.as_str().strip_prefix("club-header-") {
            if let Ok(value) = value.to_str() {
                club_headers.push((header.to_owned(), value.to_owned()));
            }
        }
    }
    ...
    for (key, value) in request.headers().iter() {
        if let Some(header) = key.as_str().strip_prefix("club-meta-") {
            if let Ok(value) = value.to_str() {
...
#[instrument(level = "debug", skip_all, fields(workspace, club_key))]
pub async fn put(request: HttpRequest, payload: Payload) -> HandlerResult<HttpResponse> {
    ...
    let part_data = PartData {
        ...
        headers: Some(headers.club_headers.clone().into_iter().collect()),
    ...
    if uploaded.deduplicated {
        response.insert_header(("club-Deduplicated", "true"));
    } else {
        if let Some(parts_count) = uploaded.parts_count {
            response.insert_header(("club-S3-Parts-Count", parts_count.to_string()));
        }
    }

    for (key, value) in headers.club_headers {
