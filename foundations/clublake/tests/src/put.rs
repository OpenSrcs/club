...
pub async fn put_new_deduplicated_no_multipart() -> eyre::Result<()> {
    ...
    check_eq!(None, res.header("club-deduplicated"));
    ...
    let res = http
        ...
        .await?;
    check!(res.status().is_success());
    check_eq!(Some("true"), res.header("club-deduplicated"));
...
pub async fn put_new_sized_no_multipart(size: usize) -> eyre::Result<()> {
    ...
    check_eq!(None, res.header("club-parts-count"));
...
pub async fn put_new_sized_multipart() -> eyre::Result<()> {
    ...
    check!(res.status().is_success());
    check_eq!(Some("1"), res.header("club-s3-parts-count"));
    ...
    check!(res.status().is_success());
    check_eq!(None, res.header("club-s3-parts-count"));
    check_eq!(Some("true"), res.header("club-deduplicated"));
...
#[tanu::test("club-header-header", "header", "value1")]
#[tanu::test("club-header-HeAdEr", "header", "value2")]
#[tanu::test("CLUB-HEADER-HEADER", "header", "value3")]
#[tanu::test("CLUB-HEADER-HEADER", "HEADER", "value4")]
...
pub async fn put_with_headers() -> eyre::Result<()> {
    ...
    let res = http
        .key_put(&key)
        .body(body)
        .header("club-header-header1", "foo")
        .header("club-header-header2", "bar")
        .header("club-meta-meta1", "baz")
...
pub async fn put_with_meta() -> eyre::Result<()> {
    ...
    let res = http
        .key_put(&key)
        .body(body)
        .header("club-meta-meta1", "foo")
        .header("club-meta-meta2", "bar")
        .send()
        .await?;
    check!(res.status().is_success());
...
pub async fn put_merge_patch(
    ...
    if let Some(strategy) = strategy {
        req = req.header("club-merge-strategy", strategy);
    }
