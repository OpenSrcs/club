...
//
// Script to check and update @opensrcs/* dependencies across all Rush packages
//
...
/**
 * Check and update @opensrcs dependencies in a package.json
 */
...
/**
 * Get all @opensrcs packages and their latest versions
 */
async function fetchLatestVersions(projects) {
  const packageNames = new Set()

  // Collect all @opensrcs package names from all projects
  ...
  console.log(`\n🔍 Checking latest versions for ${packageNames.size} @opensrcs packages...\n`)
...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`
Rush @opensrcs Dependencies Updater

Usage:
  update-opensrcs-deps [options]
...
Description:
  This script checks all Rush packages for @opensrcs/* dependencies
...
Examples:
  ...
  update-opensrcs-deps --dry-run

  # Apply updates
  update-opensrcs-deps
...
`)
  ...
  console.log('🚀 Rush @opensrcs Dependencies Updater\n')
  ...
  if (Object.keys(latestVersions).length === 0) {
    console.log('\n⚠️  No @opensrcs packages found in npm registry')
    return
  }
  ...
  if (totalUpdates === 0) {
    console.log('✅ All @opensrcs dependencies are up to date!')
  } else {
    if (dryRun) {
