...
function findPlatformRigPath() {
  ...
  const possiblePaths = [path.join(cwd, 'common', 'scripts', 'node_modules', '@opensrcs', 'platform-rig')]
...
function updatePackageJson(packagePath, eslintDeps) {
  ...
  if (packageJson.name === '@opensrcs/club-rig') {
    return { updated: false, changes: [] }
  }
...
function main() {
  ...
  try {
    ...
    if (!platformRigPath) {
      ...
      console.log('   Make sure @opensrcs/club-rig is installed as a dependency.')
      return
    }
    ...
    for (const project of projects) {
      ...
      if (result.updated) {
        ...
        console.log()
      } else if (result.changes.length === 0 && project.name !== '@opensrcs/club-rig') {
        skippedCount++
      }
    }
