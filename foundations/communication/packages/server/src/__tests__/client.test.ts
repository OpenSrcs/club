...
import { MeasureContext, WorkspaceUuid, PersonUuid, Account } from '@opensrcs/core'
import { ClublakeWorkspaceClient, getWorkspaceClient } from '@opensrcs/clublake-client'
import { getClient as getAccountClient } from '@opensrcs/account-client'
import { loadMessages } from '@opensrcs/communication-shared'
import {
  CardID,
  MessageID,
  MessageMeta,
...
import { DbAdapter } from '@opensrcs/communication-sdk-types'
import { LowLevelClient } from '../client'
import { Blob } from '../blob'
import { Metadata } from '../types'

// Mock dependencies
jest.mock('@opensrcs/clublake-client')
jest.mock('@opensrcs/account-client')
jest.mock('@opensrcs/communication-shared')
jest.mock('@opensrcs/server-token', () => ({
...
describe('LowLevelClient', () => {
  ...
  let mockLakeClient: jest.Mocked<ClublakeWorkspaceClient>
  ...
  beforeEach(() => {
    ...
    mockMetadata = {
      ...
      clublakeUrl: 'http://clublake-url',
      secret: 'test-secret',
      messagesPerBlob: 100
    }

    // Mock ClublakeWorkspaceClient
    mockLakeClient = {
      find: jest.fn(),
      update: jest.fn()
    } as any
  ...
  describe('constructor', () => {
    it('should initialize LowLevelClient with correct parameters', () => {
      ...
      expect(getWorkspaceClient).toHaveBeenCalledWith(mockMetadata.clublakeUrl, workspace, 'mock-token')
    })
  })
