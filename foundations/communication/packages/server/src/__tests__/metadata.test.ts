...
import { MessageID } from '@opensrcs/communication-types'
...
describe('metadata', () => {
  ...
  describe('getMetadata', () => {
    it('should return default values when environment variables are not set', () => {
      ...
      delete process.env.CLUBLAKE_URL
      ...
      expect(metadata).toEqual({
        accountsUrl: '',
        secret: 'secret',
        clublakeUrl: 'http://club.local:8096',
        messagesPerBlob: 200
      })
    })
    ...
    it('should use CLUBLAKE_URL from environment', () => {
      process.env.CLUBLAKE_URL = 'http://custom-clublake:9000'

      const metadata = getMetadata()

      expect(metadata.clublakeUrl).toBe('http://custom-clublake:9000')
    })
    ...
    it('should use all custom environment variables', () => {
      ...
      process.env.CLUBLAKE_URL = 'http://clublake:8080'
      ...
      expect(metadata).toEqual({
        ...
        secret: 'my-secret',
        clublakeUrl: 'http://clublake:8080',
    ...
    it('should have correct type structure', () => {
      ...
      expect(metadata).toHaveProperty('clublakeUrl')
      ...
      expect(typeof metadata.clublakeUrl).toBe('string')
      expect(typeof metadata.messagesPerBlob).toBe('number')
    })
    ...
    it('should handle URLs with different protocols', () => {
      ...
      process.env.CLUBLAKE_URL = 'wss://clublake.example.com'

      const metadata = getMetadata()

      expect(metadata.accountsUrl).toBe('https://secure-accounts.com')
      expect(metadata.clublakeUrl).toBe('wss://clublake.example.com')
    })

    it('should handle URLs with ports', () => {
      process.env.ACCOUNTS_URL = 'http://localhost:3000'
      process.env.CLUBLAKE_URL = 'http://localhost:8096'

      const metadata = getMetadata()

      expect(metadata.accountsUrl).toBe('http://localhost:3000')
      expect(metadata.clublakeUrl).toBe('http://localhost:8096')
    })
  })
})
