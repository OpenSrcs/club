...
export function isAppletAttachment (attachment: Attachment): attachment is AppletAttachment {
  return attachment.mimeType.startsWith('application/vnd.club.applet.')
}
...
export function isAppletAttachmentType (mimeType: string): boolean {
  return mimeType.startsWith('application/vnd.club.applet.')
}
...
export async function loadMessagesGroups (client: ClublakeWorkspaceClient, cardId: CardID): Promise<MessagesGroup[]> {
