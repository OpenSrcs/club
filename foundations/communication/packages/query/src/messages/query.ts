...
import { MessageProcessor } from '@opensrcs/communication-shared'
...
export class MessagesQuery implements PagedQuery<Message, MessageQueryParams> {
  ...
  constructor (
    ...
    private readonly clublake: ClublakeWorkspaceClient,
  ...
  private async loadGroups (): Promise<void> {
    ...
    const promise = (async (): Promise<MessagesGroup[]> => {
      try {
        const res = await this.clublake.getJson<MessagesGroupsDoc>(`${this.params.cardId}/messages/groups`, {
          maxRetries: 3,
          isRetryable: () => true,
          delayStrategy: {
            getDelay: () => 500
  ...
  async translate (): Promise<void> {
    ...
    const promise = async (): Promise<void> => {
      while (this.translateBlobs.length > 0) {
        ...
        const translates = await loadTranslatedMessages(this.clublake, this.params.cardId, blob, lang)
  ...
  private async loadMessagesFromBlob (blobId: BlobID): Promise<Message[]> {
    ...
    return await loadMessages(this.clublake, this.params.cardId, blobId, params, this.options)
  }
