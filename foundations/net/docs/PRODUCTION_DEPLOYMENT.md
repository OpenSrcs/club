...
Complete guide for deploying club Virtual Network to production environments.
...
services:
  network-server:
    image: opensrcs/network-pod:latest
    container_name: club-network
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: agent
          image: your-registry/club-agent:latest
          env:
            - name: NETWORK_HOST
              value: 'club-network-service'
            - name: NETWORK_PORT
              value: '3737'
            - name: AGENT_ID
              valueFrom:
...
```bash
# Create namespace
kubectl create namespace club-network

# Apply configurations
kubectl apply -f k8s/network-deployment.yaml -n club-network
kubectl apply -f k8s/agent-deployment.yaml -n club-network

# Check status
kubectl get pods -n club-network
kubectl get services -n club-network

# View logs
kubectl logs -f deployment/club-network -n club-network
kubectl logs -f deployment/club-agents -n club-network

# Scale manually
kubectl scale deployment club-agents --replicas=10 -n club-network
```
...
# Logging
LOG_LEVEL=info
LOG_FILE=/var/log/club/network.log
```
...
const requestCounter = new Counter({
  name: 'club_requests_total',
  help: 'Total number of requests',
  labelNames: ['operation', 'status']
})

const requestDuration = new Histogram({
  name: 'club_request_duration_seconds',
  help: 'Request duration in seconds',
  labelNames: ['operation']
})

const activeContainers = new Gauge({
  name: 'club_active_containers',
...
const logger = winston.createLogger({
  ...
  defaultMeta: {
    service: 'club-network',
    environment: process.env.NODE_ENV
  },
  transports: [
    new winston.transports.Console(),
    new winston.transports.File({
      filename: '/var/log/club/error.log',
      level: 'error'
    }),
    new winston.transports.File({
      filename: '/var/log/club/combined.log'
    })
  ]
})
...
kubectl top pods -n club-network

# Scale down if needed
kubectl scale deployment club-agents --replicas=5
```
...
kubectl set image deployment/club-network \
  network=opensrcs/network-pod:v2.0.0 \
  --record -n club-network
