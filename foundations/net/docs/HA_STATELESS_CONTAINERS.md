...
This feature enables club Network to support stateless containers with automatic failover capabilities, allowing you to build highly available services that must ensure only one instance is active at any given time.
...
```typescript
import { AgentImpl, containerUuid, TickManagerImpl } from '@opensrcs/network-core'
...
4. **Single network dependency**: All agents must connect to the same club Network instance
