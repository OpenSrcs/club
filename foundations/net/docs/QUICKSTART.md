...
Get started with club Virtual Network in under 10 minutes!
...
docker pull opensrcs/network-pod:latest

# Run the network server
docker run -d \
  --name club-network \
  -p 3737:3737 \
  opensrcs/network-pod:latest
```

### Option 3: NPM Packages (Coming Soon)

```bash
npm install @opensrcs/network-core \
            @opensrcs/network-client \
            @opensrcs/network-server
```
...
import type { GetOptions, ContainerUuid } from '@opensrcs/network-core'
...
- [GitHub Repository](https://github.com/opensrcs/club.net)
