# club Server - Copilot Context

## Project Overview

**Monorepo**: Rush-managed backend infrastructure for OpenSrcs Club  
**Stack**: TypeScript, Node.js v20.11.0+, pnpm 10.15.1, Rush 5.158.1, Jest  
**License**: Eclipse Public License 2.0 (OpenSrcs.)

## Package Categories

- **Core**: core, server, client, middleware
- **Database**: mongo, postgres, elastic
- **Storage**: server-storage, minio, s3, datalake, clublake
- **Infrastructure**: kafka, collaboration

Standard structure: `src/` → `lib/` (compiled), `tsconfig.json`, `jest.config.js`, `package.json` with `@opensrcs/` scope
...
- Imports: `@opensrcs/` namespace, named imports, grouped (external/internal/types)
- Errors: `PlatformError` from `@opensrcs/club`, `unknownError` wrapper
