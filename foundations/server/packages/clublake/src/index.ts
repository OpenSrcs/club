...
import { type ClublakeClient, type PutOptions, getClient } from '@opensrcs/clublake-client'
...
import serverToken, { generateToken } from '@opensrcs/server-token'
 ...
 * @public
 */
export interface ClublakeConfig extends StorageConfig {
  kind: 'clublake'
}
 ...
 * @public
 */
export function createClublakeClient (cfg: ClublakeConfig, token: string): ClublakeClient {
  const endpoint = Number.isInteger(cfg.port) ? `${cfg.endpoint}:${cfg.port}` : cfg.endpoint
  return getClient(endpoint, token)
}

export const CONFIG_KIND = 'clublake'

/**
 * @public
 */
export interface ClublakeClientOptions {
  retryCount?: number
 ...
 * @public
 */
export class ClublakeService implements StorageAdapter {
  private readonly client: ClublakeClient
  private readonly retryCount: number
  private readonly retryInterval: number

  constructor (
    readonly cfg: ClublakeConfig,
    readonly options: ClublakeClientOptions = {}
  ) {
    const secret = getMetadata(serverToken.metadata.Secret)
    if (secret === undefined) {
      console.warn('Server secret not set, clublake storage adapter initialized with default secret')
    }
    const token = generateToken(systemAccountUuid, undefined)
    this.client = createClublakeClient(cfg, token)
...
export function processConfigFromEnv (storageConfig: StorageConfiguration): string | undefined {
  const endpoint = process.env.CLUBLAKE_ENDPOINT
  if (endpoint === undefined) {
    return 'CLUBLAKE_ENDPOINT'
  }

  const config: ClublakeConfig = {
    kind: 'clublake',
    name: 'clublake',
    endpoint
  }
  ...
  storageConfig.default = 'clublake'
}
