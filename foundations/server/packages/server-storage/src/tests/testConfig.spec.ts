import type { MinioConfig } from '@opensrcs/minio'
import type { S3Config } from '@opensrcs/s3'
import type { StorageConfiguration } from '@opensrcs/server-core'
...
describe('config-parse', () => {
  ...
  it('test-decode unexpected symbols', async () => {
    const cfg: StorageConfiguration = { default: '', storages: [] }
    parseStorageEnv(
      'minio|localhost:9000?accessKey=%F0%9F%91%85%F0%9F%91%BB%20-%20%D0%AD%D0%A2%D0%9E%20%20%20%20%D1%82%D0%B0%D0%BA%D0%BE%D0%B9%20%D0%BF%D0%B0%D1%80%D0%BE%D0%BB%D1%8C%0A%D0%90%20%D1%82%D0%BE&secretKey=minio2&downloadUrl=https%3A%2F%2Ffront.club.opensrcs.org',
      cfg
    )
    expect(cfg.default).toEqual('minio')
    const minio = cfg.storages[0] as MinioConfig
    ...
    expect(minio.secretKey).toEqual('minio2')
    expect((minio as any).downloadUrl).toEqual('https://front.club.opensrcs.org')
  })
})
