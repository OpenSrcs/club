import { CONFIG_KIND as DATALAKE_CONFIG_KIND, DatalakeService, type DatalakeConfig } from '@opensrcs/datalake'
import { CONFIG_KIND as CLUBLAKE_CONFIG_KIND, ClublakeService, type ClublakeConfig } from '@opensrcs/clublake'
import { CONFIG_KIND as MINIO_CONFIG_KIND, MinioConfig, MinioService, addMinioFallback } from '@opensrcs/minio'
import { CONFIG_KIND as S3_CONFIG_KIND, S3Service, type S3Config } from '@opensrcs/s3'
import { StorageAdapter, StorageConfiguration, type StorageConfig } from '@opensrcs/server-core'
...
export function createStorageFromConfig (config: StorageConfig): StorageAdapter {
  ...
  } else if (kind === CLUBLAKE_CONFIG_KIND) {
    const c = config as ClublakeConfig
    if (c.endpoint == null) {
      throw new Error('Endpoint value is not specified')
    }
    adapter = new ClublakeService(c)
  } else {
