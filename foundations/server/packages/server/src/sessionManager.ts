...
import platform, { PlatformError, Severity, Status, UNAUTHORIZED, unknownError } from '@opensrcs/club'
import {
  type HelloRequest,
  type HelloResponse,
  type RateLimitInfo,
...
import { generateToken, type Token } from '@opensrcs/server-token'
  ...
  async close (ctx: MeasureContext, ws: ConnectionSocket, workspaceUuid: WorkspaceUuid): Promise<void> {
    ...
    if (sessionRef !== undefined) {
      ...
      ctx.info('bye happen', {
        ...
        user: sessionRef.session.getSocialIds().find((it) => it.type !== SocialIdType.CLUB)?.value,
    ...
    try {
      ...
      if (LOGGING_ENABLED) {
        ctx.info('hello happen', {
          ...
          user: service.getSocialIds().find((it) => it.type !== SocialIdType.CLUB)?.value,
