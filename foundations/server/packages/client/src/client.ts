...
import { type Client, type LoadModelResponse, type Tx } from '@opensrcs/core'
import { addLocation, getMetadata, getResource, setMetadata } from '@opensrcs/club'
...
export async function createClient (
  ...
  setMetadata(client.metadata.ClientSocketFactory, (url) => {
    const socket = new WebSocket(url, {
      headers: {
        'User-Agent': getMetadata(plugin.metadata.UserAgent) ?? 'Club Client'
      }
    })
    return socket
  })
  addLocation(clientId, () => import('@opensrcs/client-resources'))
