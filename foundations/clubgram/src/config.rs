...
pub mod clubrs {
    ...
    pub static CONFIG: LazyLock<clubrs::Config> = LazyLock::new(|| match clubrs::Config::auto() {
        Ok(config) => config,
        Err(error) => {
    ...
    pub static SERVICES: LazyLock<clubrs::ServiceFactory> =
        LazyLock::new(|| clubrs::ServiceFactory::new(CONFIG.clone()));
}
...
pub static CONFIG: LazyLock<Config> = LazyLock::new(|| {
    ...
    let settings = builder
        .add_source(config::Environment::with_prefix("CLUB"))
        .build()
