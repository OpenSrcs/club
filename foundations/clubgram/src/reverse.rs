...
use anyhow::Result;
use clubrs::services::transactor::{
    TransactionValue,
    comm::{
...
use clubrs::services::core::tx::TxDomainEvent;
...
use crate::{
    config::{CONFIG, clubrs::CONFIG as hconfig},
...
impl ReverseEvent {
    pub fn club_message_id(&self) -> String {
        match self {
            ...
            match event.event.r#type {
                MessageRequestType::CreateMessage => {
                    ...
                    let from_telegram = create_message
                        .extra
                        .as_ref()
                        .map(|e| e.get(export::CLUBGRAM_ORIGIN) == Some(&TELEGRAM))
                        .unwrap_or(false);
