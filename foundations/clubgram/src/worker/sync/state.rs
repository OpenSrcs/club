...
use clubrs::services::core::WorkspaceUuid;
...
impl KeyPrefixes for SyncInfo {
    ...
    fn with_reverse_prefix(&self, s: &str) -> String {
        format_reverse_prefix(self.club_workspace_id, &self.club_card_id, s)
    }
}
...
impl SyncState {
    ...
    pub async fn set_message(&self, telegram_id: i32, club_message: ClubMessage) -> Result<()> {
        let mut redis = self.redis.clone();

        let repr = (club_message.id.clone(), club_message.date.timestamp());
    ...
    // club by telegram
    pub async fn get_h_message(&self, telegram_id: i32) -> Result<Option<ClubMessage>> {
        let mut redis = self.redis.clone();
    ...
    pub async fn get_t_message(&self, club_id: &String) -> Result<Option<i32>> {
        let mut redis = self.redis.clone();

        Ok(redis
            .hget::<_, _, Option<i32>>(self.info.with_reverse_prefix("messages"), club_id)
            .await?)
    }
