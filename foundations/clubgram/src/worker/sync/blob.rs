use anyhow::Result;
use clubrs::services::{core::WorkspaceUuid, jwt::ClaimsBuilder};
use reqwest::{
    Body, Client,
    multipart::{Form, Part},
};
...
impl BlobClient {
    pub fn new(workspace: WorkspaceUuid) -> clubrs::Result<Self> {
        let base = CONFIG
            .blob_service_path
        ...
        let token = ClaimsBuilder::default()
            ...
            .build()
            .unwrap()
            .encode(crate::config::clubrs::CONFIG.token_secret.as_ref().unwrap())?;
