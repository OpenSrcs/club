                    ...
                    if let Some(request) = message {
                        ...
                        match (&state, request) {
                            ...
                            (WorkerState::Authorized(_), WorkerRequest::Reverse(sync_info, reverse)) => {
                                let club_message = reverse.club_message_id().clone();

                                if let Err(error) = sync.handle_reverse_update(&sync_info, reverse).await {
                                    warn!(
                                        club_workspace = %sync_info.club_workspace_id,
                                        club_card = %sync_info.club_card_id,
                                        club_card_title = %sync_info.club_card_title,
                                        %club_message,
