...
dbview() {
    local CN="postgresql://root@club.local:26257/defaultdb?sslmode=disable"
    local QUERY="SELECT workspace, namespace, key, convert_from(value,'UTF8') AS value, encode(md5,'hex') AS md5 FROM clubkvs.kvs ORDER BY namespace, key;"
    local result
