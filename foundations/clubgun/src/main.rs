...
async fn process_event(consumer: &Consumer, message: &BorrowedMessage<'_>) {
    ...
    static EVENTS: LazyLock<Counter<u64>> = LazyLock::new(|| {
        global::meter("clubgun")
            .u64_counter("clubgun_event")
            .build()
    });

    static ERRORS: LazyLock<Counter<u64>> = LazyLock::new(|| {
        global::meter("clubgun")
            .u64_counter("clubgun_error")
            .build()
    });
...
fn initialize_tracing() {
    ...
    let otel_config = otel::OtelConfig {
        mode: config::clubrs::CONFIG.otel_mode.clone(),
    ...
    let filter = Targets::default()
        .with_target(env!("CARGO_BIN_NAME"), config::clubrs::CONFIG.log)
        .with_target("actix", Level::WARN);
    let format = tracing_subscriber::fmt::layer().compact();

    match &config::clubrs::CONFIG.otel_mode {
        otel::OtelMode::Off => {
            ...
            tracing_subscriber::registry()
                ...
                .with(otel::tracer_provider(&otel_config).map(|provider| {
                    let filter = Targets::default()
                        ...
                        .with_target(env!("CARGO_PKG_NAME"), config::clubrs::CONFIG.log);

                    OpenTelemetryLayer::new(provider.tracer("clubgun")).with_filter(filter)
                }))
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    config.set(
        "bootstrap.servers",
        config::clubrs::CONFIG.kafka_bootstrap_servers(),
    );
    ...
    if let Some(debug) = config::clubrs::CONFIG.kafka_rdkafka_debug.as_ref() {
