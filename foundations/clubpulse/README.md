...
Clubpulse is a service that enables clients to share information on a “whiteboard”. Clients connected to the same “whiteboard” see data provided by other clients to the whiteboard.
...
- Input
	...
	- `CLUB-TTL` — autodelete in N seconds
	- or `CLUB-EXPIRE-AT` — autodelete in UnixTime
...
## Special cargo build options
    - "auth" (default) - use club-authorization
    Disable auth:
...
Pre-build docker images is available at: opensrcs/service_clubpulse:{tag}.

You can use the following command to run the image locally:
```bash
docker run -p 8095:8095 -it --rm opensrcs/service_clubpulse:{tag}
```

Run from source using Redis:
```bash
CLUB_REDIS_URLS=redis://club.local:6379 cargo run
```

Run from source in in-memory mode:
```bash
CLUB_BACKEND=memory cargo run
```

If you want to run the service as a part of local club development environment use the following command:
```bash
 export CLUB_REDIS_URLS="redis://club.local:6379"
 docker run --rm -it --network dev_default -p 8095:8095 opensrcs/service_clubpulse:{tag}
```
This will run Clubpulse in the same network as the rest of club services, and set the redis connection string to the one matching the local dev redis instance.

You can then access clubpulse at http://localhost:8095.


## Authetication
Clubpulse uses bearer JWT token authetication. At the moment, it will accept any token signed by the clubpulse secret. The secret is set in the environment variable CLUB_TOKEN_SECRET variable. 

## Configuration
The following environment variables are used to configure clubpulse:
   ...
   - ```CLUB_BACKEND```: storage backend "redis" or "memory" (default: "redis")
   - ```CLUB_REDIS_URLS```: redis connection string (default: redis://club.local:6379)
   - ```CLUB_REDIS_PASSWORD```: redis password (default: "&lt;invalid&gt;")
   - ```CLUB_REDIS_MODE```: redis mode "direct" or "sentinel" (default: "direct")
   - ```CLUB_REDIS_SERVICE```: redis service (default: "mymaster")
   - ```CLUB_MAX_TTL```: maximum storage time (default: 3600)
   - TODO: ```CLUB_PAYLOAD_SIZE_LIMIT```: maximum size of the payload (default: 2Mb)

## Todo (in no particular order)
- [ ] Optional value encryption
- [ ] Support for open telemetry
- [ ] Concurrency control for database migration (several instances of clubpulse are updated at the same time)
- [ ] TLS support
