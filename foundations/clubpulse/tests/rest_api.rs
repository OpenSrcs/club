...
async fn engine(
    ...
    for h in headers {
        if let Some((mut k, v)) = h.split_once(':') {
            k = k.trim();
            if k != "IF-MATCH" && k != "IF-NON-MATCH" && k != "CLUB-TTL" {
                panic!("Only `IF-MATCH`, `IF-NON-MATCH`, `CLUB-TTL` headers allowed");
            }
...
async fn put_and_get() {
    ...
    delete_any(&base, &client, workspace, key).await;
    // delete only tag
    put(&base, &client, workspace, key, data, &["CLUB-TTL: 2"]).await;
    delete_not(
        &base,
        &client,
        workspace,
    ...
    // let r = engine("put", &base, &client, &workspace, &key, "Next Another Data", &["CLUB-TTL: 2","IF-NON-MATCH: c7bcabf6b98a220f2f4888a18d01568d"]).await;
    // println!("ALL: {:?}", r);

    // test TTL
    delete_any(&base, &client, workspace, key).await;
    put(&base, &client, workspace, key, data, &["CLUB-TTL: 7"]).await;
    let ttl = get_ttl(&base, &client, workspace, key).await;
    assert!(ttl == 7 || ttl == 6, "TTL should be 7 (ok, may by 6)");
    put(&base, &client, workspace, key, data, &["CLUB-TTL: 1"]).await;
    let ttl = get_ttl(&base, &client, workspace, key).await;
