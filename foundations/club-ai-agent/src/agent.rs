...
use crate::{
    ...
    tools::{
        ...
        club::create_club_tool_set, notes::NotesToolSet, task::TaskToolSet, web::WebToolSet,
    },
};
    ...
    async fn init_tools(
        ...
        macro_rules! add_tool_set {
            ($tool_set:expr) => {
                ...
                let static_context: String = tool_set.get_static_context(&config).await;
                tool_context.push_str(&static_context);
            };
        }

        add_tool_set!(create_club_tool_set(config, context).await?);
