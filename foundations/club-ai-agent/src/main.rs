...
use anyhow::Result;
use anyhow::bail;
use club::fetch_server_config;
use clubrs::ServiceFactory;
use clubrs::services::account::LoginParams;
use clubrs::services::account::SelectWorkspaceParams;
use clubrs::services::account::WorkspaceKind;
use clubrs::services::core::SocialIdType;
use clubrs::services::event::Class;
use clubrs::services::transactor::TransactorClient;
use clubrs::services::transactor::backend::http::HttpBackend;
use clubrs::services::transactor::document::DocumentClient;
use clubrs::services::transactor::document::FindOptionsBuilder;
...
use crate::context::ClubAccountInfo;
use crate::club::blob::BlobClient;
use crate::club::types::Person;
use crate::club::typing::TypingClient;
use crate::task::Task;
...
) -> Result<(ClubAccountInfo, TransactorClient<HttpBackend>)> {
...
async fn assistant_login(
    ...
    let social_id = account_client
        .find_social_id_by_social_key(&format!("club-assistant:{}", account_uuid), true)
        .await?
        .unwrap();
    ...
    let main_social_id = social_ids
        .into_iter()
        .find(|social_id| social_id.base.r#type == SocialIdType::Club)
        .unwrap();
...
async fn main() -> Result<()> {
    ...
    tracing::debug!("base_url: {}", config.club.base_url);

    let server_config = fetch_server_config(config.club.base_url.clone()).await?;
    let clubrs_config = clubrs::ConfigBuilder::default()
        .account_service(server_config.accounts_url.clone())
        .pulse_service(server_config.pulse_url.clone())
        .log(config.log_level)
        .build()?;

    let service_factory = ServiceFactory::new(clubrs_config);
