...
use base64::Engine;
use clubrs::services::transactor::document::{DocumentClient, FindOptionsBuilder};
...
use crate::{
    ...
    context::{AgentContext, ClubAccountInfo},
    database::DbClient,
...
pub async fn prepare_system_prompt(
    ...
    let workspace_dir = config
        ...
        .unwrap()
        .replace("\\", "/");
    let personality = if let Some(person) = &config.club.person {
        format!(
...
pub async fn create_context(
    ...
    if result_context.contains("${RGB_ROLES}") {
        let rgb_roles = if let Some(person) = &config.club.person {
            format!(
