// Copyright В© 2025 Club Labs. Use of this source code is governed by the MIT license.
        ...
        Ok(Self {
            ...
            http_client: reqwest::Client::builder()
                .default_headers({
                    ...
                    headers.insert("HTTP-Referer", "https://club.io".parse().unwrap());
                    headers.insert("X-Title", "Club".parse().unwrap());
                    headers
                })
                .build()?,
        })
...
impl ProviderClient for Client {
    async fn send_messages(
        ...
        let request = self
            ...
            .await?;
        if std::env::var_os("CLUB_AI_AGENT_TRACE_REQUEST").is_some() {
            std::fs::write(
                "request.json",
