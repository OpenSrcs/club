...
impl BlobClient {
    pub fn new(
        ...
        } else if let Some(url) = &config.clublake_url {
            LakeProvider::Clublake(Url::parse(&format!("{url}/api/${workspace}",))?)
        } else {
            anyhow::bail!("Clublake URL is not configured")
        };
    ...
    pub async fn upload_file(
        ...
        let request = match &self.upload_url {
            ...
            LakeProvider::Clublake(url) => self
                .http
