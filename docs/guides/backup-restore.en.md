...
Download a workspace backup and restore it into another club instance —
...
> existing hosted club forks instead of self-hosting. We're glad to help
...
2. **Restore** — replay that backup into the destination workspace — either
   with the ready-made [club-selfhost](https://github.com/opensrcs/club-selfhost)
...
- **Copy download script** (+ **Copy token**) — copies a small shell script
  ...
  `CLUB_BACKUP_TOKEN` environment variable, or prompts for it when you run it.
...
## Step 2. Restore into a club instance

You'll need a club instance you control to restore into — either
[club-selfhost](https://github.com/opensrcs/club-selfhost), a
self-hosted deployment built from this monorepo, or another hosted instance
where you have admin access.

### Option A: club-selfhost scripts (recommended)

The [club-selfhost](https://github.com/opensrcs/club-selfhost)
repository ships helper scripts that wrap the admin tool with the right
connection settings read from your `club_v7.conf` (created by `./setup.sh`).
Make sure the stack is running (`docker compose up -d`), then from the
`club-selfhost` folder:

- [`restore-workspace.sh`](https://github.com/opensrcs/club-selfhost/blob/main/restore-workspace.sh)
...
- [`backup-restore.sh`](https://github.com/opensrcs/club-selfhost/blob/main/backup-restore.sh)
...
- [`run-tool.sh`](https://github.com/opensrcs/club-selfhost/blob/main/run-tool.sh)
...
of the platform admin tool (`@opensrcs/tool`, `dev/tool` in this repo).
...
- **club-selfhost deployment** — use
  [`run-tool.sh`](https://github.com/opensrcs/club-selfhost/blob/main/run-tool.sh):
  it starts the `opensrcs/tool` container with all connection settings
  taken from your `club_v7.conf`. Mount the backup folder into the container
...
- **Other Docker setups** — run the `opensrcs/tool` image on your stack's
...
Our team is happy to help with migration. Join the [club community](https://link.opensrcs.org/slack) to ask questions and get updates, or email us directly at [hafizmoazkhalid@gmail.com](mailto:hafizmoazkhalid@gmail.com).
...
- **Members are missing after restore** — re-run with `--accounts` and a
  ...
  club-selfhost scripts do this by default — check you didn't pass
  `--no-accounts`.)
...
- **Restore fails with a model/version error** — add `--upgrade` if the
  ...
  club-selfhost `backup-restore.sh` upgrades the workspace after restore by
  default.)
- **`Config not found: club_v7.conf`** — the club-selfhost scripts read
  connection settings from the config created by `./setup.sh`; run it first.
- **`Network ... not found`** — the club-selfhost scripts need the stack
...
- [club-selfhost](https://github.com/opensrcs/club-selfhost) — official
  self-hosted distribution, including
  [`restore-workspace.sh`](https://github.com/opensrcs/club-selfhost/blob/main/restore-workspace.sh),
  [`backup-restore.sh`](https://github.com/opensrcs/club-selfhost/blob/main/backup-restore.sh) and
  [`run-tool.sh`](https://github.com/opensrcs/club-selfhost/blob/main/run-tool.sh).
