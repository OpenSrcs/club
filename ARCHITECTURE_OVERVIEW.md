# Club Architecture Overview

## Service Overview

OpenSrcs Club consists of **30+ microservices** working together in a distributed architecture. Services are organized into functional layers for core business logic, data storage, real-time communication, media processing, and supporting features.
...
| **clublake** | 8096 | Storage adapter API. Provides S3-compatible interface for accessing stored objects. |
| **clubkvs** | 8094 | Key-value store service. Fast storage for configuration, preferences, and cached data. |
...
| **clubpulse** | 8099 | WebSocket notification server. Handles real-time push notifications to connected clients. |
| **clubgun** | - | Event processor. Consumes and processes events from Redpanda for real-time updates. |
...
| Service | Port | Description |
|---------|------|-------------|
| **front** | 8087/8088 | Web application server. Serves the club UI, handles static assets and client-side routing. |
...
| **redis** | 6379 | In-memory cache and pub/sub. Used by ClubPulse for real-time notifications. |
...
graph TB
    ...
    subgraph "Storage Services"
        ...
        Clublake[Clublake<br/>:8096<br/>Storage API]
        ClubKVS[ClubKVS<br/>:8094<br/>Key-Value]
    end
    ...
    subgraph "Real-time"
        ...
        Pulse[ClubPulse<br/>:8099<br/>WebSocket]
        Gun[ClubGun<br/>Events]
    end
...
graph LR
    ...
    subgraph "Event Consumers"
        ...
        Process[Process<br/>Automation]
        Gun[ClubGun<br/>Event Processor]
...
graph TB
    ...
    subgraph "Storage APIs"
        ...
        Clublake[Clublake :8096<br/>Storage Adapter]
    end
    ...
    Clublake -->|Metadata| CockroachDB
    Clublake -->|Access Blobs| Minio
...
|---------|-----------|------|---------|--------------|
| **Frontend** | | | | |
| front | opensrcs/front | 8087/8088 | Web application server | account, transactor, collaborator, datalake |
| **Core** | | | | |
| account | opensrcs/account | 3000 | Authentication & user management | cockroach, redpanda, stats |
| transactor | opensrcs/transactor | 3332 | Transaction processing (WebSocket) | cockroach, redpanda, fulltext, account |
| workspace | opensrcs/workspace | - | Workspace management | cockroach, redpanda, minio, account |
| stats | opensrcs/stats | 4900 | Metrics collection | - |
| **Storage** | | | | |
| datalake | opensrcs/datalake | 4030 | Blob storage & metadata | cockroach, minio, account |
| clublake | opensrcs/clublake | 8096 | Storage adapter API | cockroach, minio |
| clubkvs | opensrcs/clubkvs | 8094 | Key-value store | cockroach |
| **Search** | | | | |
| fulltext | opensrcs/fulltext | 4702 | Full-text search indexing | elasticsearch, cockroach, rekoni, redpanda |
| rekoni | opensrcs/rekoni-service | 4004 | Document intelligence | stats |
| **Real-time** | | | | |
| collaborator | opensrcs/collaborator | 3078 | Real-time document collaboration | account, datalake, transactor |
| clubpulse | opensrcs/clubpulse | 8099 | WebSocket notifications | redis |
| clubgun | opensrcs/clubgun | - | Event processor | redpanda, account |
| **Media** | | | | |
| stream | opensrcs/stream | 1080 | Video streaming | datalake, redpanda |
| media | opensrcs/media | - | Media processing | redpanda, account |
| preview | opensrcs/preview | 4040 | Thumbnail generation | datalake |
| **Features** | | | | |
| print | opensrcs/print | 4005 | PDF generation | cockroach, minio, account |
| sign | opensrcs/sign | 4006 | Digital signatures | cockroach, minio, account |
| payment | opensrcs/payment | 3040 | Payment processing | account |
| export | opensrcs/export | 4009 | Data export | cockroach, minio, account |
| analytics | opensrcs/analytics-collector | 4017 | Analytics collection | account, stats |
| process | opensrcs/process | - | Workflow automation | redpanda, account |
| rating | opensrcs/rating | - | Content rating | cockroach, redpanda, account |
| **Backup** | | | | |
| backup | opensrcs/backup | - | Automated backup | cockroach, minio, account |
| backup-api | opensrcs/backup-api | 4039 | Backup REST API | minio, account |
| **Primary Database** | | | | |
...
- `ACCOUNTS_URL`: `http://club.local:3000` - Account service URL
- `STATS_URL`: `http://club.local:4900` - Metrics collection URL
...
- `FULLTEXT_DB_URL`: `http://club.local:9200` - Elasticsearch URL
- `CLUB_DB_CONNECTION`: CockroachDB connection for club* services
...
- `CLUB_KAFKA_BOOTSTRAP`: `redpanda:9092` - Kafka bootstrap servers

### Service URLs (Internal)
- `ACCOUNTS_URL`: `http://club.local:3000`
- `TRANSACTOR_URL`: `ws://club.local:3332`
- `FULLTEXT_URL`: `http://club.local:4702`
- `REKONI_URL`: `http://club.local:4004`
- `COLLABORATOR_URL`: `ws://club.local:3078`
- `DATALAKE_URL`: `http://club.local:4030`
- `CLUBLAKE_URL`: `http://club.local:8096`
- `PULSE_URL`: `ws://club.local:8099/ws`
- `PREVIEW_URL`: `http://club.local:4040`
- `STREAM_URL`: `http://club.local:1080/recording`
- `PAYMENT_URL`: `http://club.local:3040`
- `PRINT_URL`: `http://club.local:4005`
- `SIGN_URL`: `http://club.local:4006`
- `BACKUP_URL`: `http://club.local:4039/api/backup`
- `AI_BOT_URL`: `http://club.local:4010`

### Frontend Configuration
- `FILES_URL`: `http://club.local:4030/blob/:workspace/:blobId/:filename` - File download URL pattern
- `FRONT_URL`: `http://club.local:8087` - Frontend base URL
- `BRANDING_URL`: `http://club.local:8087/branding.json`
- `DESKTOP_UPDATES_URL`: `https://dist.opensrcs.org`

### Authentication & Security
- `CLUB_TOKEN_SECRET`: `secret` - Token signing for club services
...
### Redis Configuration (ClubPulse)
- `CLUB_REDIS_URLS`: `redis://redis:6379`
- `CLUB_BIND_PORT`: `8099`

### Stream Service
- `STREAM_ENDPOINT_URL`: `datalake://club.local:4030`
